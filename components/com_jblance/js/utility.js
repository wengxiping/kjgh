...
var checkUsername = function(el){
	...
	//if(inputstr.length > 0){
		var myRequest = jQuery.ajax({
            ...
            method: "POST",
			data: {"inputstr":inputstr, "name":name},
			beforeSend: function(){ jQuery("#status_"+name).empty().removeAttr("class").addClass("jbloading dis-inl-blk"); },
			success: function(response) {
				var resp = jQuery.parseJSON(response);
				if(resp["result"] == "OK"){
					jQuery("#status_"+name).removeClass("jbloading failurebg").addClass("successbg");
...
var createDropzone = function(elementId, paramName, mockFile, parameters){
	...
	var myDropzone = new Dropzone("#"+elementId, { // Make the whole body a dropzone
		url: "index.php?option=com_jblance&task="+parameters.uploadTask+"&"+JoomBriToken, // Set the url
		paramName: paramName,
		//autoDiscover : false,
	...
	myDropzone.on("removedfile", function(file){
		...
		if(file.previewTemplate.children[4]){
			...
			var myRequest = jQuery.ajax({
				url: "index.php?option=com_jblance&task="+parameters.removeTask+"&"+JoomBriToken,
				method: "POST",
