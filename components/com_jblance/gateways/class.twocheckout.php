...
class twocheckout_class {
	...
	function twocheckoutPayment(){
		...
		$this->add_field('mode', "2CO");
		
		$this->add_field('merchant_order_id', $invoiceNo);
		//$this->add_field('total', $totamt);
		$this->add_field('li_0_product_id', $item_num);
		$this->add_field('li_0_name', $itemname);
		$this->add_field('li_0_description', $itemname);	
		$this->add_field('li_0_price', $totamt);
	...
	function twocheckoutReturn($data){
		...
		if($this->validate_ipn($data)){
			$invoice_num = array_key_exists('merchant_order_id', $data) ? $data['merchant_order_id'] : '';		// get the invoice number from the post variable
			$return['success'] = true;
