...
class JblanceControllerMembership extends JControllerLegacy {
		...
		if(!empty($gwayName)){
			...
			if(class_exists($className)){
				...
				if($isValid){
					...
					if($invoice_num){
						$identify 	= JblanceHelper::identifyDepositOrPlan($invoice_num);
						$result  	= array_merge($result, $identify);
					...
					$data['jblance_failure_reason'] = JText::_('COM_JBLANCE_PAYMENT_VERIFICATION_FAILED').' : '.$result['failure_reason'];
				}
				
				//Trigger the plugin event to feed the activity - buyer pick freelancer
				JPluginHelper::importPlugin('joombri');
				$dispatcher = JEventDispatcher::getInstance();
				$dispatcher->trigger('onReturnAfterPayment', array($result, $row));
	...
	function saveDepositFund(){
		...
		$row->tax_percent_dep 	= $taxpercent;
		$total					= $row->amount + $depositFeeFixed + ($depositFeePerc*$row->amount)/100 + ($taxpercent*$row->amount)/100;
	...
	function releaseEscrow(){
		...
		JPluginHelper::importPlugin('joombri');
		$dispatcher = JEventDispatcher::getInstance();
		$dispatcher->trigger('onBuyerReleaseEscrow', array($escrow->from_id, $escrow->to_id, $escrow->project_id, $escrow->type, $escrow->id));
	...
	function acceptEscrow(){
		...
		JPluginHelper::importPlugin('joombri');
		$dispatcher = JEventDispatcher::getInstance();
		$dispatcher->trigger('onFreelancerAcceptEscrow', array($escrow->to_id, $escrow->from_id, $escrow->project_id, $escrow->type, $escrow->id));
	...
	function saveWithdrawFund(){
		...
		$row->finalAmount = $post['amount'] - ($post['withdrawFeeFixed'] + ($post['amount'] * $post['withdrawFeePerc'] / 100));
