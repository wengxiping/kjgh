...
class JblanceControllerMessage extends JControllerLegacy {
	...
	function sendMessage(){
		...
		//set the parent msg to unread for sorting by "unread" in inbox
		if($message->save($post)){
			$query = "UPDATE #__jblance_message SET is_read=0 WHERE id=".(int)$parent_id;
			$db->setQuery($query);
			if(!$db->execute()){
		...
		//Trigger the plugin event - after user send private message
		JPluginHelper::importPlugin('joombri');
		$dispatcher = JEventDispatcher::getInstance();
		$dispatcher->trigger('onUserSendPrivateMessage', array($message->idFrom, $message->idTo, $message->id));
