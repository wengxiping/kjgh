 ...
 */
	...
	function grabUsergroupInfo(){
        ...
        $new_ugid = $session->get('ugid','','register');
        $this->systemLog(array('get参数ugid:'.$ugid,'session中:'.$new_ugid));
	...
	function saveUserNew(){
		...
		if($user->id == 0){
			...
			$userInfo['groups'] = $defaultUserGroup;
			...
			if(($useractivation == 1 || $useractivation == 2) && !$requireApproval){
				...
				$userInfo['activation'] = JApplicationHelper::getHash(JUserHelper::genRandomPassword());
				$userInfo['block'] = 1;
			}

			// Bind the post array to the user object
			if(!$usern->bind($userInfo)){
			...
			$user->groups = array_unique($merged);
			if(!$user->save()){
		...
		$dispatcher->trigger('onUserCreateProfile', array($userid));
    ...
    //系统日志--- 操作
	function systemLog($arr){
	    date_default_timezone_set('PRC');
        $resource = fopen('systemLog','a+');
        fwrite($resource,implode(',',$arr)."----".date("Y-m-d H:i:s")."\r\n");
        fclose($resource);
    }
}
