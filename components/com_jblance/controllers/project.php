...
class JblanceControllerProject extends JControllerLegacy {
	...
	function saveProject(){
		...
		if($isNew){
			...
			if($chargePerProject > 0){
				$post['profit_perproject'] = $chargePerProject;
	...
	function saveBid(){
		...
		if($isNew){
			...
			if($chargePerBid > 0){
				$post['profit_perbid'] = $chargePerBid;
	...
	function acceptBid(){
		...
		$bid->p_status = 'COM_JBLANCE_IN_PROGRESS';
...
//		$project->status = 'COM_JBLANCE_CLOSED';
        $project->status = 'COM_JBLANCE_FROZEN';
	...
	function repostProject(){
		...
		if($chargePerProject > 0){
			$project->profit_perproject += $chargePerProject;
	...
	function saveInviteUser(){
		...
		$invite_ids	= ArrayHelper::toInteger($invite_ids);
