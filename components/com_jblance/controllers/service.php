...
class JblanceControllerService extends JControllerLegacy {
 	...
 	function saveService(){
 		...
 		if($isNew){
 			...
 			if($chargePerService > 0){
 				$post['profit_perservice'] = $chargePerService;
 	...
 	function placeOrder(){
 		...
 		if($user->guest){
 			...
 			$returnUrl ='index.php?option=com_jblance&view=service&layout=viewservice&id='.$service_id;
 		...
 		$extras	= $app->input->get('extras', null, 'array');
 		$clean = JblanceHelper::cleanParams($extras);
 		$registry = new JRegistry();
 		$registry->loadArray($clean);
 		$post['extras'] = $registry->toString();
