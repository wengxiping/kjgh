...
<form action="<?php echo JRoute::_('index.php'); ?>" method="post" name="userForm" id="userForm" class="form-validate form-horizontal" onsubmit="return validateForm(this)">
		...
		<table class="table table-bordered table-hover table-striped">
			...
			for($i=0, $n=count($this->paymodes); $i < $n; $i++){
				...
				<tr>
					...
					<td>
						<?php 
						$feeString = JblanceHelper::formatCurrency($mode->withdrawFeeFixed);
						if($mode->withdrawFeePerc > 0)
							$feeString = $feeString.' + '.$mode->withdrawFeePerc.'%';
						echo $feeString; ?>
					</td>
				</tr>
			<?php
	...
	<?php elseif($step == 2) : ?>
		...
		<div class="control-group">
			...
			<div class="controls">
				<?php 
				$feeString = JblanceHelper::formatCurrency($gwInfo->withdrawFeeFixed);
				if($gwInfo->withdrawFeePerc > 0)
					$feeString = $feeString.' + '.$gwInfo->withdrawFeePerc.'%';
				echo $feeString; ?>
				<input type="hidden" name="withdrawFeeFixed" value="<?php echo $gwInfo->withdrawFeeFixed; ?>" />
				<input type="hidden" name="withdrawFeePerc" value="<?php echo $gwInfo->withdrawFeePerc; ?>" />
			</div>
		</div>
		<?php
