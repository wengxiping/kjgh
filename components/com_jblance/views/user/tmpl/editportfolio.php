 ...
 */
 ...
 $portfolioImageMockFile = '{}';
 if($this->row->id > 0){
	 $isJson = JblanceHelper::toJson($this->row->picture);
	 $portfolio_image = JBMediaHelper::processAttachment($isJson, 'portfolio', false);
	 $registry = new JRegistry();
	 $registry->loadArray($portfolio_image);
	 $portfolioImageMockFile = $registry->toString();
 }
...
jQuery(document).ready(function($){
	...
	<?php if($this->row->id > 0 || $type == 'addnew'){ ?>
	...
	//}
		var params = { 'maxFileSize' : 2, 'maxFiles' : 5, 'uploadTask' : 'ajax.dzuploadfile', 'removeTask' : 'ajax.dzremovefile', 'acceptedFiles' : 'image/*', 'uploadType' : 'portfolio'};	//todo: add size and max files to config
		createDropzone('portfolio-image', 'portfolioImage', '<?php echo $portfolioImageMockFile; ?>', params);
	<?php } ?>
...
<form action="<?php echo JRoute::_('index.php'); ?>" method="post" name="userFormPortfolio" id="userFormPortfolio" class="form-validate form-horizontal" onsubmit="return validateForm(this);" enctype="multipart/form-data" novalidate>
	...
	<?php 
 		...
 		echo '<p class="alert alert-info">'.JText::_('COM_JBLANCE_NO_PORTFOLIO').'</p>';
	?>
...
<?php if(/* count($this->portfolios) == 0 ||  */$this->row->id > 0 || ($type == 'addnew' && $allowedPortfolio > 0)){ ?>
	...
	<div class="control-group">
		<label class="control-label"><?php echo JText::_('COM_JBLANCE_PORTFOLIO_IMAGE'); ?> :</label>
		<div class="controls">
			<?php echo JBMediaHelper::renderDropzone('portfolio-image'); ?>
		</div>
	</div>
	<div class="control-group">
		...
		<div class="controls">
			...
			<ul class="unstyled">
				...
				<li style="padding: 5px 0px;">
					<div class="row-fluid">
						<div class="span6">
							<button type="button" id="portfolioattachment<?php echo $i; ?>" class="btn"><i class="jbf-icon-file"></i> <?php echo JText::_('COM_JBLANCE_ATTACH_FILE').' - '.$i; ?></button>
