...
<form action="<?php echo JRoute::_('index.php'); ?>" method="post" name="userForm">
	...
	<div class="row-fluid">
		...
		<?php 
			$isJson = JblanceHelper::toJson($row->picture);
			echo JBMediaHelper::renderImageCarousel($isJson, 'portfolio'); ?>
		</div>
		<?php 
		} ?>
		...
		for($i=1; $i<=5; $i++){
			...
			if($row->$attachmentColumnNum){ 
				$count++;
			?>
		<p><i class="jbf-icon-download"></i> <?php echo LinkHelper::getPortfolioDownloadLink('portfolio', $this->row->id, 'user.download', $attachmentColumnNum); ?></p>
		<?php 
			}
		} 
		if($count == 0)
