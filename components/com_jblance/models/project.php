...
class JblanceModelProject extends JModelLegacy
{
    ...
    function getListProject()
    {
        ...
        $categoryShow = self::categoryIdShow($app->input->get('Itemid',0));
...
//        $limit = 2;
    ...
    function getPickUser()
    {
        ...
        $query = "SELECT b.*,u.username,u.name,p.project_title,p.project_type,p.commitment FROM #__jblance_bid b " .
            ...
            "WHERE b.project_id =" . $db->quote($id) . " and b.status<>'COM_JBLANCE_DENIED' AND TRUE";
    ...
    function getRateUser()
    {
        ...
        $isOwnedOperation = JblanceHelper::checkOwnershipOfOperation($project_id, 'projectrating');
        if (!$isOwnedOperation) {
            $msg = JText::sprintf('COM_JBLANCE_NOT_AUTHORIZED_TO_ACCESS_THIS_PAGE');
            $link = JRoute::_('index.php?option=com_jblance&view=user&layout=dashboard', false);
            $app->enqueueMessage($msg, 'error');
    ...
    function getSearchProject()
    {
        ...
        $id_locate = ArrayHelper::toInteger($id_locate);

        //set the variables into state variables
        $this->setState('keyword', $keyword);
        $this->setState('phrase', $phrase);
        $this->setState('id_categ', $id_categ);
        $this->setState('project_type', $proj_type);
        $this->setState('id_location', $id_locate);
        $this->setState('budget', $budget);
        $this->setState('status', $status);
        $this->setState('type', $type);

        //if the type is parentcategory, get the array of children
        if ($type == 'parentcategory') {
            $category = JTable::getInstance('category', 'Table');
            $id_categ = $category->getChildren($id_categ[0]);
            $this->setState('id_categ', $id_categ);
        }
        ...
        if (count($id_locate) > 0 && ($id_locate[0] > 1)) {
            ...
            $children_id = ArrayHelper::toInteger($children_id);    //sanitize array
    ...
    {
        $db = JFactory::getDbo();
        $query = "SELECT categorylist FROM #__menu where id=" . $itemId;
        $db->setQuery($query);//echo $query;
        $ret = $db->loadResult();

        if ($ret) {
            $ret = rtrim($ret,',');
            return 'p.id_category in (' . $ret . ')';
        } else {
            return false;
        }
    }
