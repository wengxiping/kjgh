 ...
 */
 ...
 class JblanceModelUser extends JModelLegacy {
 	...
 	function getDashboard(){
		...
		$registry->loadString($userInfo->ug_params);
 	...
 	function pendingActions($userid){
 		...
 		if($bid_offers > 0){
 			$link = JHtml::_('link', $link_my_bid, '<i class="jbf-icon-search"></i>', array());
 		...
 		if($tot_proj > 0){
 			$link = JHtml::_('link', $link_my_project, '<i class="jbf-icon-search"></i>', array());
 		...
 		if($invites > 0){
 			$link = JHtml::_('link', $link_project_list, '<i class="jbf-icon-search"></i>', array());
 		...
 		if($service_not_started > 0){
 			$link = JHtml::_('link', $link_my_service, '<i class="jbf-icon-search"></i>', array());
 		...
 		if($userType->freelancer){
 			...
 			if($seller_rating > 0){
 				$link = JHtml::_('link', $link_svc_sold, '<i class="jbf-icon-search"></i>', array());
 		...
 		elseif($userType->buyer){
 			...
 			if($buyer_rating > 0){
 				$link = JHtml::_('link', $link_svc_bought, '<i class="jbf-icon-search"></i>', array());
 		...
 		if($enableEscrowPayment){
	 		...
	 		if($in_escrow > 0){
	 			$link = JHtml::_('link', $link_managepay, '<i class="jbf-icon-search"></i>', array());
