 ...
 */
 ...
 class JblanceModelMembership extends JModelLegacy {
	...
	function getPlanAdd(){
        ...
        $ugid = $session->get('ugid','','register');
		if(empty($ugid)){
	...
	function getWithdrawFund(){
		...
		$query = "SELECT * FROM #__jblance_paymode ".
				 "WHERE published=1 AND is_withdraw=1 ".
				 "ORDER BY ordering";
		$db->setQuery($query);
	...
	function getEscrow(){
		...
		if(empty($projects)){
			...
			$lists 	= JHtml::_('select.genericlist', $types, 'project_id', "class='span4 required' size='8' onchange='fillProjectInfo();'", 'value', 'text', '');
		}
	...
	function getInvoice(){
		...
		elseif($type == 'project'){
			...
			if($usertype == 'freelancer'){
				$query = "SELECT p.*, p.accept_date AS invoiceDate,p.id AS invoiceNo,p.lancer_commission AS commission_amount,u.email,u.name,ju.biz_name,ju.address,ju.id_location,ju.postcode FROM #__jblance_project p ".
			...
			elseif($usertype == 'buyer'){
				$query = "SELECT p.*, p.accept_date AS invoiceDate,p.id AS invoiceNo,p.buyer_commission AS commission_amount,u.email,u.name,ju.biz_name,ju.address,ju.id_location,ju.postcode FROM #__jblance_project p ".
		...
		elseif($type == 'service'){
			$usertype 	= $app->input->get('usertype', '', 'string');
			if($usertype == 'freelancer'){
				$query = "SELECT so.*,s.service_title,so.order_date AS invoiceDate,so.id AS invoiceNo,u.email,u.name,ju.biz_name,ju.address,ju.id_location,ju.postcode FROM #__jblance_service_order so ".
						 "LEFT JOIN #__jblance_service s ON s.id=so.service_id ".
	...
	function getNewPaymode($var, $default, $disabled, $type){
        $app = JFactory::getApplication();
        $db	= JFactory::getDbo();
        $html = '';
        $gatewayType = 'TRUE';
	...
	function buildPlanInfo($planId){
		...
		if($ugInfo->allowBidProjects){
			...
			$infos[$i]->value = ($planInfo->portfolioCount == 0) ? "<i class='jbf-icon-remove'></i>" : $planInfo->portfolioCount;
			$i++;
