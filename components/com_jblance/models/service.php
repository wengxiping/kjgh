...
class JblanceModelService extends JModelLegacy
{
    ...
    function getMyService()
    {
        ...
        $query = "SELECT s.*,(SELECT COUNT(*) FROM #__jblance_service_order so WHERE so.service_id=s.id) buycount,(select group_concat(c.category) from #__jblance_category c WHERE find_in_set(c.id,s.id_category) group by c.published) categoryLiist FROM #__jblance_service s" .
...
//        $queryCategory = self::categoryIdShow($app->input->get('Itemid',0));
//        if ($queryCategory) {
//            $queryStrings[] = $queryCategory;
//            unset($queryCategory);
//        }

        $where = implode(' AND ', $queryStrings);

        $query = "SELECT DISTINCT s.*,(select group_concat(xx.category) from #__jblance_category xx WHERE find_in_set(xx.id,s.id_category) group by xx.published) categoryList,(select count(*) from #__jblance_service_order yy where s.id=yy.service_id) sell_nums,(select uu.id_location from #__jblance_user uu where s.user_id=uu.user_id) id_location FROM #__jblance_service s " .
    ...
    function getRateService()
    {
        ...
        $isOwnedOperation = JblanceHelper::checkOwnershipOfOperation($order_id, 'servicerating');
        if (!$isOwnedOperation) {
            $msg = JText::sprintf('COM_JBLANCE_NOT_AUTHORIZED_TO_ACCESS_THIS_PAGE');
            $link = JRoute::_('index.php?option=com_jblance&view=user&layout=dashboard', false);
            $app->enqueueMessage($msg, 'error');
        ...
        $query = " SELECT s.service_title,so.id order_id,s.user_id seller_id,so.user_id buyer_id,xx.id_location,tt.name as group_name FROM #__jblance_service_order so" .
            " LEFT JOIN #__jblance_service s ON so.service_id=s.id" .
            " LEFT JOIN #__jblance_user xx ON xx.user_id=so.user_id" .
            " LEFT JOIN #__jblance_usergroup tt ON tt.id=xx.ug_id" .
    ...
    public static function categoryIdShow($itemId)
    {
        $db = JFactory::getDbo();
        $query = "SELECT categoryservicelist FROM #__menu where id=" . $itemId;
        $db->setQuery($query);//echo $query;
        $ret = $db->loadResult();
        if ($ret) {
            $ret = rtrim($ret,',');
            return 's.id_category in (' . $ret. ')';
        } else {
            return false;
        }
    }
