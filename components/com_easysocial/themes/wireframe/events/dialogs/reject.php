...
<dialog>
	...
	<selectors type="json">
	{
		...
		"{rejectForm}": "[data-reject-event-form]"
	}
	</selectors>
	...
	<title><?php echo JText::_('COM_EASYSOCIAL_EVENTS_DIALOG_REJECT_REQUEST_TITLE'); ?></title>
	<content>
		<p><?php echo JText::sprintf("COM_EASYSOCIAL_EVENTS_DIALOG_REJECT_REQUEST_CONTENT", $guest->getName(), $event->getName());?></p>

		<form data-reject-event-form method="post" action="<?php echo JRoute::_('index.php');?>">
			<input type="hidden" name="id" value="<?php echo $event->id;?>" />
			<input type="hidden" name="userId" value="<?php echo $guest->id;?>" />
			<input type="hidden" name="controller" value="events" />
			<input type="hidden" name="task" value="rejectGuest" />
