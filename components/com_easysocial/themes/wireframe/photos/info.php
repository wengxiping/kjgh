	...
	<small>
		...
		<?php if ($this->config->get('photos.location')) { ?>
			...
			if (!empty($location)) {
				...
				if (!empty($address)) {
			?>
				<span data-photo-location class="es-photo-location"><?php echo JText::_("COM_EASYSOCIAL_PHOTOS_TAKEN_AT"); ?> <u data-popbox="module://easysocial/locations/popbox" data-lat="<?php echo $location->latitude; ?>" data-lng="<?php echo $location->longitude; ?>" data-location-provider="<?php echo $this->config->get('location.provider'); ?>">
				<a href="<?php echo $location->getMapUrl(); ?>" target="_blank"><?php echo $address; ?></a></u></span>.
				<?php } ?>
			<?php } ?>
		<?php } ?>
	</small>
