...
if ( $this->config->get('display_categories') && is_array($this->categories)):
	...
	echo '<div class="category row-fluid">';
	...
	?>
	<div
		class="mt-index-cat-card span3"
		style="background-image: url('<?php
		echo $this->config->getjconf('live_site') . $this->config->get('relative_path_to_cat_original_image') . rawurlencode($cat->cat_image); ?>')" alt="<?php echo htmlspecialchars($cat->cat_name); ?>">
	<?php
	} else {
	?>
	<div class="mt-index-cat-no-image-card span3">
		<?php
		}
	...
	$sub_cat_count = count($this->sub_cats[$cat->cat_id]);
	if (isset($this->sub_cats) && isset($this->sub_cats[$cat->cat_id]) && $sub_cat_count > 0) {
		$j = 0;
		...
		foreach ($this->sub_cats[$cat->cat_id] AS $sub_cat):
			...
			$htmlCatDetails .= '</span>';
			$htmlCatDetails .= '</a>';
			if( $j < ($sub_cat_count-1) ) {
				$htmlCatDetails .= '<span class="mt-index-subcats-separator">' . $itemsSeparatorString . '</span>';
			}
			$j++;
	...
	if( !empty($htmlCatDetails) ) {
		echo '<div class="mt-index-cat-details span9">';
		echo $htmlCatDetails;
		echo '</div>';
	}
