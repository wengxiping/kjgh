	...
	function submitbutton(pressbutton) {
		...
		if (form.rev_text.value == ""){
			...
			if(
				...
				&&
				(
					$this->my->authorise('mtree.listing.rate', 'com_mtree')
					||
					($this->config->get('prevent_rate_own_listing') == '1' && $this->my->id > 0 && $this->my->id != $this->link->user_id)
				)
			) {
...
<h1 class="contentheading"><?php echo JText::_( 'COM_MTREE_WRITE_REVIEW' ) . ' - ' . $this->link->link_name; ?></h1>
...
<div id="listing">
	...
	<form action="<?php echo JRoute::_('index.php'); ?>" method="post" name="mtForm" id="mtForm" class="form-horizontal row-fluid">
		...
		<div class="controls controls-row">
			<button type="button" onclick="javascript:submitbutton('addreview')" class="btn btn-primary"><?php echo JText::_( 'COM_MTREE_ADD_REVIEW' ) ?></button>
