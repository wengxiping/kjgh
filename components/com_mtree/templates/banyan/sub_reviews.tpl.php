<?php
$profilepicture_loaded = jimport('profilepicture.profilepicture');

// Don't hide Vote Helpful by default
if( !isset($hide_vote_helpful) ) {
	$hide_vote_helpful = false;
}
?>
<div class="row-fluid">
	<div id="reviews" class="reviews">
		<?php if( !isset($hide_title) || $hide_title == false ) { ?>
			<div class="title"><?php echo JText::_( 'COM_MTREE_REVIEWS' ); ?> (<?php echo $this->total_reviews; ?>)<?php echo ($this->mtconf['show_listingreviewsrss']) ? $this->plugin('showrssfeed','reviews') : ''; ?></div>
				...
				<div itemprop="review" itemscope itemtype="http://schema.org/Review" class="review row-fluid">
					<div class="review-head span2">
						<div class="review-info">
							...
							<span itemprop="author" itemscope itemtype="http://schema.org/Person"><span itemprop="name" class="review-owner">
					<?php echo ( ($review->user_id) ? '<a href="' . JRoute::_('index.php?option=com_mtree&amp;task=viewusersreview&amp;user_id='.$review->user_id) . '">' . $review->username . '</a>': $review->guest_name); ?>
							...
							<p class="review-date">
								<time itemprop="datePublished" datetime="<?php echo strftime('%Y-%m-%d', strtotime($review->rev_date)); ?>">
					...
					<div class="span10">
						<div class="review-text">
							<div class="review-title"><?php
								...
								?>
								<span itemprop="name"><?php echo $review->rev_title; ?></span></div>
							<span itemprop="reviewBody"><?php echo trim($review->rev_text); ?></span><?php
						...
						<div class="row-fluid">
							<div class="span6">
								...
								if( $hide_vote_helpful == false && $this->my->id > 0 && $this->mtconf['user_vote_review'] == 1 ) {
							...
							<div class="span6">
								<div class="review-reply-report-permalink">
									<?php
									if(  $this->my->authorise('mtree.listing.report_review', 'com_mtree') || ( $this->my->id == $this->link->user_id && empty($review->ownersreply_text) ))
									{
										// User is authorised to make a review report
										if( $this->my->authorise('mtree.listing.report_review', 'com_mtree') )
										{
										...
										// (1) This is the user's own listing; (2) The owner's hasn't made a reply yet; (3) Configuration is set to allow owner to reply to review.
		...
		<?php else:
			...
			if( !isset($hide_submitreview) || $hide_submitreview == false ) { ?>
				<p>
					<a href="<?php echo JRoute::_('index.php?option=com_mtree&task=writereview&link_id='.$this->link->link_id); ?>" class="btn">
						<span class="icon-edit"></span>
						<?php echo MTEXT::_( 'BE_THE_FIRST_TO_REVIEW', $this->tlcat_id ); ?>
					</a>
				</p>
			<?php
			}
