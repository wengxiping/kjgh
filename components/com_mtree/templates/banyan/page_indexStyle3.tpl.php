	...
	if ( $this->config->get('display_categories') && is_array($this->categories)):
		...
		foreach ($this->categories as $cat):
			...
			<h2><?php
				...
				echo '<span class="mt-index-cat-name">';
				echo htmlspecialchars($cat->cat_name);
				echo '</span>';

				if($displayIndexCatCount) {
					echo '<span class="mt-index-cat-num-of-cats">' . MText::sprintf('X_CATEGORIES', $this->tlcat_id, $cat->cat_cats) . '</span>';
				}
				if($displayIndexListingCount) {
					echo '<span class="mt-index-cat-num-of-listings">' . MText::sprintf('X_LISTINGS', $this->tlcat_id, $cat->cat_links) . '</span>';
				}
			...
			if(!empty($cat->cat_desc) && $this->config->getTemParam('displayCatDesc','0')){
				...
				$htmlCatDetails .= '<a class="mt-index-cat-overlay-link" href="' . $urlCategory . '">';
				$htmlCatDetails .= '</a>';
