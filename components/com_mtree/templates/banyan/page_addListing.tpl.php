...
<div class="mt-add-listing">

	<h1 class="contentheading"><?php echo ($this->link->link_id) ? JText::_( 'COM_MTREE_PAGE_HEADER_EDIT_LISTING' ) : 	JText::_( 'COM_MTREE_PAGE_HEADER_ADD_LISTING' ); ?></h1>
	...
	<form action="<?php echo JRoute::_("index.php") ?>" method="post" enctype="multipart/form-data" name="mtForm" id="mtForm" class="form-horizontal form-validate">
		...
		<?php if( $this->warn_expiring == 1 ) { ?>
			<p class="text-warning"><?php
				echo JText::sprintf( 'COM_MTREE_YOUR_LISTING_IS_EXPIRING_SOON', $this->days_remaining, $this->mtconf['days_to_expire'] );
				?></p>
		<?php } ?>
		...
		<div id="mtfields" class="mt-add-listing-fields">
			<?php
			$this->fields->resetPointer();
			while( $this->fields->hasNext() ) {
				$field = $this->fields->getField();
		...
		<?php if( $this->mtconf['use_map'] == 1 ) { ?>
			...
			<div id="mapcon">
				...
				<script type="text/javascript">
					...
					<?php if( $this->mtconf['map_provider'] !=  'google') { ?>
					var mapConfig = {
					map_provider: '<?php echo $this->mtconf['map_provider']; ?>' ,
					mapbox_access_token: '<?php echo $this->mtconf['mapbox_access_token']; ?>',
					here_app_id: '<?php echo $this->mtconf['here_app_id']; ?>',
					here_app_code: '<?php echo $this->mtconf['here_app_code']; ?>',
					<?php if ( !is_null(json_decode($this->mtconf['custom_tile_server'])) ) { ?>
					custom_tile_server: <?php echo $this->mtconf['custom_tile_server']; ?>,
					<?php } else { ?>
						custom_tile_server: false,
					<?php }?>
					custom_tile_url: '<?php echo $this->mtconf['custom_tile_url']; ?>',
					custom_tile_attribution: '<?php echo $this->mtconf['custom_tile_attribution']; ?>'
				}
				<?php } ?>
				</script>
