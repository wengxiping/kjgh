<div class="listing-summary <?php echo $span; ?> row-fluid<?php

$showImageInSummary = $this->config->getTemParam('showImageInSummary',1);

echo ($showImageInSummary ? ' mt-ls-has-image':' mt-ls-no-image');
...
?>" data-link-id="<?php echo $link->link_id; ?>">
	...
	?>
	<div class="summary-view-details<?php echo ($showImageInSummary ? ' mt-ls-has-image':' mt-ls-no-image')?>">
		<?php
		...
		$focus2 = $link_fields->getFieldById( $config['summary_view']['focus_field_2'] );

		if(
				isset($focus1) && $focus1->hasValue()
				||
				isset($focus2) && $focus2->hasValue()
				||
				$this->config->get('show_review')
		)
		{
			?>
			<div class="mt-ls-fields mt-ls-field-focus">
				<?php
				if( isset($focus1) && $focus1->hasValue() ) {
					?>
				...
				if( isset($focus2) && $focus2->hasValue() ) {
					?>
					<div class="<?php echo $focus2->getFieldTypeClassName(); ?>">
		...
		// Main Attributes
		if( !empty($config['summary_view']['main_attr_fields']) )
		{
			$facilitiesHtml = '';
			foreach($config['summary_view']['main_attr_fields'] AS $cf_id) {
				$field = $link_fields->getFieldById($cf_id);
				if(!is_null($field) && $field->hasValue()) {

					$facilitiesHtml .= '<div id="field_'.$field->getId().'" class="mt-ls-field '.$field->getFieldTypeClassName().'" >';
					$facilitiesHtml .= '<div class="output">';
					$facilitiesHtml .= $field->getOutput();
					$facilitiesHtml .= '</div>';
					$facilitiesHtml .= '<div class="caption">';
					$facilitiesHtml .= $field->getCaption();
					$facilitiesHtml .= '</div>';
					$facilitiesHtml .= '</div>';
				}
			}

			if(!empty($facilitiesHtml)) {
				?>
				<div class="mt-ls-fields mt-ls-field-main-attr">
					<?php echo $facilitiesHtml; ?>
				</div>
				<?php
			} else {
				?><div class="mt-ls-spacing"></div><?php
			}
		}

		echo '<div class=mt-ls-spacing></div>';
