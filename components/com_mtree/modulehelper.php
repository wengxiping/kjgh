<?php
/**
 * @copyright	Copyright (C) 2011-present Mosets Consulting. All rights reserved.
...
class MTModuleHelper
{
	...
	public static function getItemid() {
		...
		if( empty($items) )
		{
			$items	= $menu->getItems('link', 'index.php?option=com_mtree&view=listall&cat_id=0');
		}
	...
	public static function filterUnauthorisedCategories( $cats )
	{
		if (is_null($cats) )
		{
			return $cats;
		...
		foreach( $cats AS $cat)
		{
			if (in_array($cat->cat_id, $authorised_cat_ids) )
			{
				$cats_results[] = $cat;
			}
		}
