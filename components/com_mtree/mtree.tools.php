...
function loadLink( $link_id, &$savantConf, &$fields, &$params ) {
	...
	if(!empty((array)$link)==0) return false;
...
function assignCommonViewlinkVar( &$savant, &$link, &$fields, &$pathWay, &$params ) {
	...
	$savant->assign('hide_back_to_listing_link', false);
	...
	// Map
	assignGoogleMaps($savant, $mtconf);
...
function assignCommonListlinksVar( &$savant, &$links, $pathWay, &$pageNav ) {
	...
	if( !empty($links) )
	{
		foreach( $links AS $link ) {
			...
			if( !empty($link->images) ) {
				$images = explode(',',$link->images);
				$link->link_image = $images[0];
				$link->images = $images;
			}
		}
	}
	...
	if( is_array($links) && count($links) > 0 ) {
		foreach( $links AS $link ) {
			...
			$links_fields[$link->link_id]->setCoresValue( $link->link_name, $link->link_desc, $link->firstname, $link->lastname, $link->address, $link->city, $link->state, $link->country, $link->postcode, $link->contactperson, $link->mobile, $link->date, $link->year, $link->telephone, $link->fax, $link->email, $link->website, $link->price, $link->link_hits, $link->link_votes, $link->link_rating, $link->link_featured, $link->link_created, $link->link_modified, $link->link_visited, $link->publish_up, $link->publish_down, $link->metakey, $link->metadesc, $link->user_id, $link->username );
 ...
 * assignClusterMapVar
 *
 * Assign common Savant2 variables used in cluster maps.
 */

function assignClusterMapVar( &$savant, &$links )
{
	...
	if( !empty($links) )
	{
		foreach ($links AS $link)
		{
			...
			{
				$strLocation = '['
						. json_encode($link->link_name) . ','
						. $link->lat . ','
						. $link->lng . ',';

				if(isset($link->link_image)) {
					$strLocation .= '\'' . $link->link_image . '\',';
				} else {
					$strLocation .= '\'\',';
				}

				$strLocation .= '\'' . JRoute::_('index.php?option=com_mtree&task=viewlink&link_id='.$link->link_id.'&Itemid='.$Itemid) . '\''
						. ']';
				$arrLocations[] = $strLocation;
			}
		}
	}

	$savant->assign( 'arrLocations', $arrLocations );
...
function assignGoogleMaps(&$savant, $mtconf) {
	// Map types
	$map_types = explode('|',$mtconf->get('google_maps_type_ids'));
	array_walk($map_types, function(&$value) { $value = 'google.maps.MapTypeId.'.$value; });

	// Styled Map
	$use_styled_map = false;
	$google_maps_styled_map_style_array = '';
	if(
		$mtconf->get('google_maps_styled_map_style_array') != ''
		&&
		in_array('google.maps.MapTypeId.STYLED_MAP', $map_types)
	) {
		$use_styled_map = true;
		$map_types[] = '"map_style"';
		$google_maps_styled_map_style_array = $mtconf->get('google_maps_styled_map_style_array');
	}

	$savant->assign( 'map_types', $map_types );
	$savant->assign( 'use_styled_map', $use_styled_map );
	$savant->assign( 'google_maps_styled_map_style_array', $google_maps_styled_map_style_array );

	// Which map to show by default?
	if( $mtconf->get('google_maps_type_id', 'ROADMAP') == 'STYLED_MAP' ) {
		$savant->assign( 'default_map', '\'styled_map\'' );
	} else {
		$default_map = 'google.maps.MapTypeId.'.$mtconf->get('google_maps_type_id', 'ROADMAP');
		$savant->assign( 'default_map', $default_map );
	}
}

function assignGoogleMapsAPIKeyURLParam( &$savant, $mtconf )
{
	$google_maps_api_key_url_param = '';

	if( $mtconf->get('google_maps_api_key') != '' ) {
		$google_maps_api_key_url_param = '&key='.$mtconf->get('google_maps_api_key');
	}

	$savant->assign( 'google_maps_api_key_url_param', $google_maps_api_key_url_param );
}
...
function getCatsSelectlist( $cat_id=0, &$cat_tree, $max_level=0 ) {
	...
	// is returned. This is only checked for top level categories (ie: cat_parent == 0).
	if( $cat_id == 0 )
	{
	...
	{
		$authorised_categories = $cat_ids;
	}


	if ( count($authorised_categories) > 0 )
	{
		$level++;
		
		if( $max_level == 0 || $level <= $max_level)
		{
			foreach( $authorised_categories AS $cid )
			{
...
function mtAppendPathWay( $option, $task, $cat_id=0, $link_id=0, $cf_id=0, $img_id=0, $rev_id=0, $user_id=0 ) {
	...
	switch( $task ) {
		case "advsearch2":
			...
			break;
		case "searchby":
			$pathway->addItem( JText::_( 'COM_MTREE_PATHWAY_SEARCHBY' ), 'index.php?option=com_mtree&task=searchby&cat_id='.$cat_id );
			if( $cf_id > 0 )
			{
				...
				if( !is_null($cf_caption) ) {
					$pathway->addItem( $cf_caption, 'index.php?option=com_mtree&task=searchby&cat_id='.$cat_id.'&cf_id='.$cf_id );
					$cf_value = JFactory::getApplication()->input->getString('value', '');
					if( !empty($cf_value) )
					{
						$pathway->addItem( $cf_value, 'index.php?option=com_mtree&task=searchby&cat_id='.$cat_id.'&cf_id='.$cf_id.'&value='.$cf_value );
					}
				}
			}
			break;
		...
		case "viewusersfav":
			$pathway->addItem( JText::_('COM_MTREE_PATHWAY_OWNER') , 'index.php?option=com_mtree&task=owner' );
 ...
 * If no assignment is available, it will attempt to look up to top most parent
...
function getRandomListingsSeed()
{
	global $mtconf;

	$current = time();
	$random_listings_shuffle_frequency = (int) $mtconf->get('random_listings_shuffle_frequency');

	return $current - ($current % $random_listings_shuffle_frequency);
}

function isAuthorisedToViewCategory($cat_id)
{
	if (is_object($cat_id) && is_numeric($cat_id->cat_id)) {
		$cat_id = $cat_id->cat_id;
	}

	$tmpMtConf = mtFactory::createConfig();
	$tmpMtConf->setCategory($cat_id);

	if( !in_array($tmpMtConf->get('category_view_access_level'),JFactory::getUser()->getAuthorisedViewLevels()) )
	{
		return false;
	}
...
function isAuthorisedToViewCurrentCategory()
{
	global $mtconf;

	if( !in_array($mtconf->get('category_view_access_level'),JFactory::getUser()->getAuthorisedViewLevels()) )
	{
		return false;
	}
...
/**
 * Check if the currently logged in user is authorised to create listing.
 *
 * @return bool
 */
function isAuthorisedToCreateListing()
{
	global $mtconf;

	$my = JFactory::getUser();

	// If user is from Super Users group, they are automatically authorised to edit listing. This can be changed in
	// 'group_ids_authorised_to_create_listing' config.
	if( count(array_intersect($mtconf->getGroupsAuthorisedToCreateListing(), $my->getAuthorisedGroups())) > 0 ) {
		return true;
	}
...
/**
 * Check if the currently logged in user is authorised to edit the given listing. Users from Super Users group
 * are granted permission to edit listings.
 *
 * @param $link Listing object
 *
 * @return bool
 */
function isAuthorisedToEditListing($link=null)
{
	global $mtconf;

	$my = JFactory::getUser();

	// Check if owners are allow to modify listing in front-end?
	// If so, verify the currently logged in user is the owner of the listing.
	if ( !is_null($link) && $my->id == $link->user_id && $mtconf->get('user_allowmodify') == 1 && $my->id > 0 ) {
		return true;
	}

	// If user is from Super Users group, they are automatically authorised to edit listing. This can be changed in
	// 'group_ids_authorised_to_edit_listing' config.
	if( count(array_intersect($mtconf->getGroupsAuthorisedToEditListing(), $my->getAuthorisedGroups())) > 0 ) {
		return true;
	}
...
/**
 * Check if the currently logged in user is authorised to delete the given listing. Users from Super Users group
 * are always granted permission to delete listings.
 *
 * @param $link Listing object
 *
 * @return bool
 */
function isAuthorisedToDeleteListing($link)
{
	global $mtconf;
	...
	// If user is from Super Users group, they are automatically authorised to edit listing. This can be changed in
	// 'group_ids_authorised_to_delete_listing' config.
	if( count(array_intersect($mtconf->getGroupsAuthorisedToDeleteListing(), $my->getAuthorisedGroups())) > 0 ) {
		return true;
	}
...
/***
 * Load the necessary javascript & CSS files for jQuery Datepick plugin
 */
function includeJavascriptCSSFileDatepick() {
	global $mtconf;
	...
	$document->addScript( JURI::root() . ltrim($mtconf->get('relative_path_to_js'),'/') . 'jquery.plugin.min.js');
	$document->addScript( JURI::root() . ltrim($mtconf->get('relative_path_to_js'),'/') . 'jquery.datepick.min.js');
	$document->addScript( JURI::root() . ltrim($mtconf->get('relative_path_to_js'),'/') . 'jquery.datepick.ext.min.js');
	$document->addScript( JURI::root() . ltrim($mtconf->get('relative_path_to_js'),'/') . 'jquery.datepick.validation.min.js');
	$document->addStyleSheet( JURI::root() . ltrim($mtconf->get('relative_path_to_js'),'/') . 'jquery.datepick.css');
