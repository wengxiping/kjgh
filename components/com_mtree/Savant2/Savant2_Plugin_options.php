...
class Savant2_Plugin_options extends Savant2_Plugin {
	 ...
	 * @access   public
	 *
	 * @internal param array $options An associative array of key-value pairs; the
	 * key is the option value, the value is the option label.
	 *
	 * @internal param array|string $selected A string or array that matches one
	 * or more option values, to tell the function what options should be
	 * marked as selected.  Defaults to an empty array.
	 *
	 * @internal param array|string $attr Extra attributes to apply to the option
	...
	function plugin()
	{
		list($options, $selected, $attr, $labelIsValue) = array_merge(func_get_args(), array(array(), array(), null, false));
