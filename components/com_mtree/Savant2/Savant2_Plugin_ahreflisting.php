	...
	function plugin()
	{
		...
		if ($show["new"] <> false) {
			...
			if ( ($jdate->toUnix()-strtotime($link->link_created)) < ($mtconf->get('link_new')*86400) ) {
				$htmlSup .= '<sup class="new">'.JText::_( 'COM_MTREE_LINK_NEW' ).'</sup> ';
			}
		}
		...
		if ($show["featured"] <> false) {
			if ( $link->link_featured ) {
				$htmlSup .= '<sup class="featured">'.JText::_( 'COM_MTREE_LINK_FEATURED' ).'</sup> ';
			}
		}
		...
		if ($show["popular"] <> false) {
			if ( ($this->datediff($link->link_created) > 0) && ($link->link_hits/$this->datediff($link->link_created)) >= $mtconf->get('link_popular') ) {
				$htmlSup .= '<sup class="popular">'.JText::_( 'COM_MTREE_LINK_POPULAR' ).'</sup> ';
			}
		}

		if( !empty($htmlSup) ) {
			$html .= '<span class="mt-listing-name-sup">';
			$html .= $htmlSup;
			$html .= '</span>';
		}
