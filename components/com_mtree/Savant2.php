...
class Savant2 {
	...
	*/
	...
	{
		global $mtconf;
		if($mtconf->get('template') != 'banyan') {
			$this->setPath('template',$mtconf->getjconf('absolute_path').'/components/com_mtree/templates/banyan/');
		...
		// the default path in which we will fallback to if a template file does not exists.
		$this->addPath('template', $conf['template_path_default']);
	...
	*/
	...
	function loadTemplate($tpl = null, $setScript = false)
	{
		...
		$document_type = (string) strtolower(JFactory::getDocument()->getType());

		// look for [filename].tpl.(json|mobi).php if the request is requesting for JSON pr mobile output
		if( $document_type  === 'json' || $document_type  === 'mobi' )
		{
			$path_parts = pathinfo($tpl);
			$tpl =  $path_parts['filename'] . '.' . $document_type . '.' . $path_parts['extension'];
		}
