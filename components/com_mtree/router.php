...
function MtreeBuildRoute(&$query) {
    ...
    if(JFactory::getApplication()->input->getString('format') == 'mobi')
    {
        return array();
    }
	...
	if(!isset($query['task']) && !isset($query['view'])) {
        ...
        if(!isset($query['Itemid'])) {
            // If there is no task/view and even Itemid, just return the query segment and don't attempt to build route.
            return $segments;
        }

        // There is an Itemid here. Let's check raw link from JMenu and see if we can get Mosets Tree menu item raw
        // link.
        $menus	    = JFactory::getApplication('site')->getMenu();
        ...
        // Yay, we found a menu item based on Itemid. Let's merge the query from this menu to the one pass to this
        // function.
	...
	switch($task) {
		...
		case 'adjacentListing':
			$mtLink = new mtLinks( $db );
			$mtLink->load( $query['link_id'] );
			$segments = array_merge($segments,appendCat( $mtLink->cat_id, $query ));
			$segments = array_merge($segments,appendListing( $mtLink->link_name, $mtLink->link_id, $mtLink->alias, false ));

			if( isset($query['direction']) && $query['direction'] == '-1' )
			{
				$segments[] =  $mtconf->get('sef_adjacentlisting_previous');
			} else {
				$segments[] =  $mtconf->get('sef_adjacentlisting_next');
			}
			unset($query['link_id']);
		...
		case 'listrandom':
		case 'listall':
		case $task == 'toplisting' && $query['type'] != 'alpha':
			if( $task == 'toplisting' ) {
				$task = "list".$query['type'];
				$type = $query['type'];
				unset($query['type']);
			} else {
		...
		case $task == 'toplisting' && $query['type'] == 'alpha':
			...
			if( isset($query['start']) )
			{
				$page = getPage($query['start'],$mtconf->get('fe_num_of_alpha'));
				$segments[] = $page;
			}
			break;
		...
		case 'viewusersfav';
			...
			if(!empty($user_id)) {
				...
				if( $mtconf->get('sef_owner_slug_type') == 1 ) {
		...
		case 'owner':
			...
			$segments[] = $mtconf->get('sef_owner');
			if( isset($query['start']) ) {
				$page = getPage($query['start'],$mtconf->get('fe_num_of_owners'));
				$segments[] =  $mtconf->get('sef_owner_page') . $page;
		...
		case 'att_download':
			$segments[] = $mtconf->get('sef_attachment');
			$segments[] = getId( 'link', $query );
			$segments[] = getId( 'cf', $query );
			break;
		...
		case 'rss':
			if( isset($query['type']) && $query['type'] == 'reviews')
			{
				$mtLink = new mtLinks($db);
				$mtLink->load($query['link_id']);
				$segments   = appendCatListing($mtLink, $query, false);
				$segments[] = $mtconf->get('sef_rss');
				$segments[] = $mtconf->get('sef_rss_listingreviews');
				unset($query['link_id']);

			} else if( isset($query['type']) && $query['type'] == 'userfavourites') {

				$username = mtFactory::getUsername($query['user_id']);
				$segments[] = $username;
				$segments[] = $mtconf->get('sef_rss');
				$segments[] = $mtconf->get('sef_rss_userfavourites');
				unset($query['user_id']);
...
function MtreeParseRoute($segments)
{
	...
	switch($end_segment) {
		...
		case $mtconf->get('sef_adjacentlisting_previous'):
		case $mtconf->get('sef_adjacentlisting_next'):
			$path_names = array_slice( $segments, 0, -1 );
			$link_id = findLinkID( $path_names );
			$vars['task'] = 'adjacentListing';
			$vars['link_id'] = $link_id;

			$vars['direction'] = '1';
			if( $end_segment == $mtconf->get('sef_adjacentlisting_previous') ) {
				$vars['direction'] = '-1';
			}
			break;
		...
		case $total_segments > 2 && $mtconf->get('sef_listalpha') == $segments[$total_segments-3]:
			...
			$vars['task'] = 'listalpha';
			if($page > 0) {
				$vars['limit'] = $mtconf->get('fe_num_of_alpha');
				$vars['limitstart'] = $mtconf->get('fe_num_of_alpha') * ($page -1);
			}
			break;

		case $total_segments == 1 && $segments[0] == $mtconf->get('sef_owner'):
		case $total_segments == 2 && $segments[0] == $mtconf->get('sef_owner') && preg_match('/' . $mtconf->get('sef_owner_page') . '[0-9]+/',$segments[1]):
		case $total_segments > 1 && $mtconf->get('sef_owner') == $segments[$total_segments-1]:
		case $total_segments > 2 && $mtconf->get('sef_owner') == $segments[$total_segments-2] && preg_match('/' . $mtconf->get('sef_owner_page') . '[0-9]+/',$segments[$total_segments-1]):
			if( $total_segments > 1 && $mtconf->get('sef_owner') == $segments[$total_segments-2] )
			{
				$vars['cat_id'] = findCatId(array_slice($segments,0,-2));
			}
			else if( $mtconf->get('sef_owner') == $segments[$total_segments-1] )
			{
				$vars['cat_id'] = findCatId(array_slice($segments,0,-1));
			}
			$vars['task'] = 'owner';

			$end_segment = $segments[$total_segments-1];
			$isPaged = preg_match( '/' . $mtconf->get('sef_owner_page') . '[0-9]+/',$end_segment);

			if ( $isPaged ) {
				// Get the page number
				$pagenumber = substr( $end_segment, strlen($mtconf->get('sef_owner_page')) );
				$vars['limit'] = $mtconf->get('fe_num_of_owners');
				$vars['limitstart'] = $mtconf->get('fe_num_of_owners') * ($pagenumber -1);
			}
			break;
		...
		case $total_segments == 2 && in_array($segments[$total_segments-2],array($mtconf->get('sef_owner'),$mtconf->get('sef_listings'),$mtconf->get('sef_reviews'),$mtconf->get('sef_favourites'))) == true:
			if( $total_segments == 2 ) {
				...
				$owner_slug = $segments[ ($total_segments-1) ];
			} else {
				$task = $segments[$total_segments-3];
				$owner_slug = $segments[ ($total_segments-2) ];
			}
		...
		switch($task) {
			...
			if( $mtconf->get('sef_owner_slug_type') == 1 ) {
				$owner_username = murldecode($owner_slug);
		...
		case $total_segments == 3 && $mtconf->get('sef_attachment') == $segments[$total_segments-3] && is_numeric($segments[$total_segments-2]) && is_numeric($segments[$total_segments-1]):
			$vars['task'] = 'att_download';
			$vars['link_id'] = $segments[$total_segments-2];
			$vars['cf_id'] = $end_segment;
			break;
		...
		case $total_segments > 1 && $mtconf->get('sef_rss') == $segments[$total_segments-2]:
			...
			if($end_segment==$mtconf->get('sef_rss_new')) {
				...
				$vars['type'] = 'updated';
			} elseif ($end_segment==$mtconf->get('sef_rss_userfavourites')) {
				$db->setQuery( "SELECT id FROM #__users WHERE username = " . $db->quote($segments[0]) . " LIMIT 1" );
				$vars['user_id'] = $db->loadResult();
				$vars['type'] = 'userfavourites';
			} elseif ($end_segment==$mtconf->get('sef_rss_listingreviews')) {
				$vars['link_id'] = findLinkId(array_slice($segments,0,-2));
				$vars['type'] = 'reviews';
			}
			break;
...
function findLinkID( $path_names ) {
	...
	switch( $mtconf->get('sef_link_slug_type') )
	{
		...
		case 3:
			if(is_numeric($link_name)) return $link_name;
