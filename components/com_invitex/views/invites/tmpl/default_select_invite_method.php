...
<div class=" top_div_icon" >
	<div class="thumbnails icon_ul  row">
	<span class="invitex_label"><i>*</i>邀请方式:</span>
		<?php
			$api_used       = '';
			$message_type   = '';
			$img_name       = '';
			...
			foreach ($this->invite_methods as $ind => $method)
			{
				...
				?>
					<span class="invitex_li " id="list_invitex_li<?php	echo $li_count;	?>" onclick="showinvitebuttondiv(this,'<?php echo $method ?>')">
						<?php if(!empty($this->tool_tip_arr[$method]))	echo $this->tool_tip_arr[$method] ?>
					</span>
				<?php
				}
			}
...
<script>
jQuery('.invitex_li').on('click',function () {
