...
function modChrome_T3Section($module, &$params, &$attribs) { 
	...
	if($params->get('color-module')) {
		$bgModule = 'mod-bg-'.$params->get('color-module');
	}

	if($params->get('bg-module')) {
		$bgimageModule = 'background-image: url('.JUri::root().''.$params->get('bg-module').');';
	}

	if($params->get('bg-position')) {
		$bgpositionModule = 'background-position:'.$params->get('bg-position');
	}


	if($params->get('color-module') || $params->get('bg-module')) {
		$modStyle = 'style="'.$bgimageModule.' '.$bgpositionModule.'"';
	}

	if($params->get('padding-bottom')) {
		$paddingBottom = 'padding-bottom';
	}

	if($params->get('padding-top')) {
		$paddingTop = 'padding-top';
	}

	if($params->get('border-bottom')) {
		$borderBottom = 'border-bottom';
	}

	if($params->get('border-top')) {
		$borderTop = 'border-top';
	}

	if($params->get('module-width') == 0) {
		$moduleWidth    = 'container-fluid';
	} elseif($params->get('module-width') == 1) {
		$moduleWidth    = 'container';
	}

	if (!empty ($module->content)) {
		$html = "<{$moduleTag} class=\"t3-section section{$moduleClassSfx} {$moduleClass} {$paddingBottom} {$paddingTop} {$borderBottom} {$borderTop} {$bgModule}\" id=\"Section{$module->id}\"  {$modStyle} >" .
					"<div class=\"{$moduleWidth}\"><div class=\"section-inner\">";

		if (($module->showtitle != 0) && ($moduleType != 'mod_ja_acm')) {
			$html .= "<div class=\"section-title\"><div class=\"sub-heading\"><span>{$moduleSub}</span></div><{$headerTag} class=\"h2 {$headerClass}\">{$module->title}</{$headerTag}></div>";
		}

		$html .= "<div class=\"section-ct\">{$module->content}</div></div></div></{$moduleTag}>";
