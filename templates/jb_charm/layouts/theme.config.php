 ...
 * Hero blocks
 */
$blocks = array_keys($config->get('hero-block', array()));

foreach ($blocks as $block) {

	$styles["block.{$block}"]  = '';
	$block_attributes[$block]   = '';
	$block_classes[$block]     = ($this['config']->get("hero-block.{$block}.class", false)) ? $this['config']->get("hero-block.{$block}.class") : '';

	if ($hero_bg = trim($this['config']->get("hero-block.{$block}.background", false))) {

		if (preg_match('/^(#([a-fA-F0-9]){3,6})|^rgb\((\d{1,3}),\s?(\d{1,3}),\s?(\d{1,3})\)/', $hero_bg, $match)) {
			$styles["block.{$block}"] = 'style="background-color: ' . $match[0] . ';"';
			$block_classes[$block]   .= ($this['config']->get("hero-block.{$block}.animation")) ? ' tm-hue-rotate' : '';
		}
		elseif(filter_var($hero_bg, FILTER_VALIDATE_URL)) {
			$styles["block.{$block}"] = 'style="background-image: url(\''.$hero_bg.'\');"';
			$block_classes[$block]   .= ' uk-cover-background';
		}
		elseif (is_file(realpath($hero_bg))) {
			$styles["block.{$block}"] = 'style="background-image: url(\''.$this['config']->get('site_url').'/'.$hero_bg.'\');"';
			$block_classes[$block]   .= ' uk-cover-background';
		}
	}

	if ($this['config']->get("hero-block.{$block}.particles", false)) {

		$block_attributes[$block] = 'data-tm-particles';

		if ($color = $this['config']->get("hero-block.{$block}.particle-color", false)) {
			$block_attributes[$block] = 'data-tm-particles="{color: \''. $color .'\'}"';
		}
	}
}

/*
 * Block Footer settings @joombri
 */
...
$this['asset']->addFile('js', 'js:lib.js');
