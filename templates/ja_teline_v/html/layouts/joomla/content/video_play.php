...
if ($ctm_source == 'other') {
	...
	if ($url) {
		...
		echo '<iframe class="ja-video" width="'.$width.'" height="'.$height.'" src="'.$url.'" style="border:none" allowfullscreen></iframe>';
	}
	...
	if ($url) {
		...
		echo '<iframe class="ja-video" width="'.$width.'" height="'.$height.'" src="'.$url.'" style="border:none" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>';
	}
} elseif($ctm_source == 'youku') {
	$url = $params->get('ctm_embed_url', '');
	if ($url) {
		if(preg_match('#.*?/id_\?v\==([^&]+).*#i', $url)){
			$vid = preg_replace('#.*?/id_\?v\==([^&]+).*#i', '$1', $url);
		} else {
			$vid = $url;
		}
