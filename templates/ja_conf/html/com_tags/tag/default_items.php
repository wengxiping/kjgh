...
<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm" class="form-inline">
			...
			<?php if ($this->params->get('filter_field')) : ?>
				<div class="input-group">
					...
					<span class="input-group-btn">
						<button aria-label="<?php echo JText::_('JSEARCH_FILTER_SUBMIT'); ?>" type="button" name="filter-search-button" title="<?php echo JText::_('JSEARCH_FILTER_SUBMIT'); ?>" onclick="document.adminForm.submit();" class="btn btn-light">
							<span class="fa fa-search" aria-hidden="true"></span>
						</button>
						<button aria-label="<?php echo JText::_('JSEARCH_FILTER_CLEAR'); ?>" type="reset" name="filter-clear-button" title="<?php echo JText::_('JSEARCH_FILTER_CLEAR'); ?>" class="btn btn-light" onclick="resetFilter(); document.adminForm.submit();">
	...
	<ul class="category tags-list media-group list-unstyled row">
		<?php foreach ($items as $i => $item) : ?>
			...
			<?php else: ?>
				<li class="media cat-list-row<?php echo $i % 2; ?> col-md-4 clearfix" >
