			...
			<ul class="unstyled logout-links">
				<?php if ( $showPrivateMessages ) { ?>
					<li class="logout-private-messages">
						<a href="<?php echo $privateMessageURL; ?>" title="<?php echo ( $newMessageCount == 1 ? CBTxt::T( 'YOU_HAVE_COUNT_NEW_PRIVATE_MESSAGE', 'You have [count] new private message.', array( '[count]' => $newMessageCount ) ) : CBTxt::T( 'YOU_HAVE_COUNT_NEW_PRIVATE_MESSAGES', 'You have [count] new private messages.', array( '[count]' => $newMessageCount ) ) ); ?>">
							...
							<?php if ( $newMessageCount ) { ?>
								<?php echo '<sup>'.$newMessageCount.'</sup>'; ?>
							<?php } ?>
						</a>
					</li>
				<?php } ?>
					...
					<li class="logout-connection-requests">
						<a href="<?php echo $_CB_framework->viewUrl( 'manageconnections' ); ?>">
							...
							<?php if ( $newConnectionRequests ) { ?>
								<?php echo '<sup>'.$newConnectionRequests.'</sup>'; ?>
							<?php } ?>
						</a>
					</li>
				<?php } ?>
