...
class plgJoomBriJblanceFeeds extends JPlugin {
	...
	function onBuyerReleaseEscrow($actorid, $targetid, $projectid, $type, $escrowid){
		$now = JFactory::getDate();
		$feedsHelper = JblanceHelper::get('helper.feeds');		// create an instance of the class FeedsHelper
		...
		if($type == 'COM_JBLANCE_PROJECT')
			$act->title = JText::sprintf("PLG_JBLANCE_BUYER_RELEASE_ESCROW_FOR_PROJECT", '{actor}', '{projectid}');
		elseif($type == 'COM_JBLANCE_SERVICE')
			$act->title = JText::sprintf("PLG_JBLANCE_BUYER_RELEASE_ESCROW_FOR_SERVICE", '{actor}', '{serviceid}');
		elseif($type == 'COM_JBLANCE_OTHER')
			$act->title = JText::sprintf("PLG_JBLANCE_BUYER_RELEASE_ESCROW", '{actor}');
	...
	function onFreelancerAcceptEscrow($actorid, $targetid, $projectid, $type, $escrowid){
		$now = JFactory::getDate();
		$feedsHelper = JblanceHelper::get('helper.feeds');		// create an instance of the class FeedsHelper
		...
		if($type == 'COM_JBLANCE_PROJECT')
			$act->title = JText::sprintf("PLG_JBLANCE_FREELANCER_ACCEPT_ESCROW_FOR_PROJECT", '{actor}', '{projectid}');
		elseif($type == 'COM_JBLANCE_SERVICE')
			$act->title = JText::sprintf("PLG_JBLANCE_FREELANCER_ACCEPT_ESCROW_FOR_SERVICE", '{actor}', '{serviceid}');
		elseif($type == 'COM_JBLANCE_OTHER')
			$act->title = JText::sprintf("PLG_JBLANCE_FREELANCER_ACCEPT_ESCROW", '{actor}');
