...
$arr_limit_cat_to = $cache->call(array('modMTListingsHelper','getCatIdFilter'), $params, $cat_id, $link_id);
$listings 	= $cache->call(array('modMTListingsHelper','getList'), $params, $arr_limit_cat_to);
...
if( $layout != 'dropdown' )
{
	...
	if( !empty($ltask) )
	{
		$first_cat_id = reset($arr_limit_cat_to);
		$show_more_link = JRoute::_( 'index.php?option=com_mtree&task='.$ltask.'&' . (($only_subcats) ? 'cat_id='.$cat_id : (($first_cat_id) ? 'cat_id='.$first_cat_id : '') ). $itemid);
	}

}

require JModuleHelper::getLayoutPath('mod_mt_listings', $params->get('layout', 'default'));