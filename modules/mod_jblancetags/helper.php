...
//*********************** tags from module parameters
	function joomulus_tagwords_sd ($set_Itemid) { // build tagwords from SD list
		...
		$now = date('Y-m-d'); 
		$where = "approved=1 AND status='COM_JBLANCE_OPEN'";
...
//******************* build flash HTML with SWFobject 
	function joomulus_createflashcode($tags, $altdiv, $subflashvars ) {
		...
		$flashtag .= 'tcolor: "0x'.str_replace("#", "", $this->options['tcolor']).'",';
		if ( $this->options['tcolor2'] == "" ) { $flashtag .= 'tcolor2: "0x'.str_replace("#", "", $this->options['tcolor']).'",'; } else { $flashtag .= 'tcolor2: "0x'.str_replace("#", "", $this->options['tcolor2']).'",'; }
		$flashtag .= 'hicolor: "0x'.str_replace("#", "", $this->options['hicolor']).'",';
	...
	function getWords($table = 'content', $colList = '*', $where = ''){
		$dbo = JFactory::getDbo();
		$query = 'SELECT '.$colList.' FROM #__'.$table;
