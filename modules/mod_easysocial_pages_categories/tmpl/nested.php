	...
	<?php if ($categories) { ?>
		...
		<li class="o-tabs__item has-notice" data-filter-item data-type="category" data-id="<?php echo $category->id;?>">
			...
			<?php if ($params->get('display_counter', true)) { ?>
				<span class="o-tabs__bubble" data-counter="<?php echo $category->getTotalCluster(SOCIAL_TYPE_PAGE);?>">
					<?php echo $category->getTotalCluster(SOCIAL_TYPE_PAGE);?>
				</span>
			<?php } ?>

			<div class="o-loader o-loader--sm"></div>
			<?php if (!empty($category->childs)) { ?>
				<?php echo ES::themes()->includeTemplate('site/helpers/categories/menu.nested', array('category' => $category, 'type' => SOCIAL_TYPE_PAGE, 'activeCategory' => '')); ?>
			<?php } ?>
		</li>
		<?php } ?>
	<?php } ?>
