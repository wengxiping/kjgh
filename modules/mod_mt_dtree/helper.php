...
class modMTDtreeHelper {
	...
	public static function getListings( $params, $categories ) {
	...
	public static function getChildrenCategories( $cat_id, $cat_level, $params ) {
		global $mtconf;
		...
		if ( $cat_level > 0  ) {
			...
			if ( count($cats) ) {
				foreach( $cats AS $cat ) {
					$children_ids = modMTDtreeHelper::getChildrenCategories( $cat->cat_id, ($cat_level-1), $params );
					$cats = array_merge( $cats, $children_ids );
				}
			}
		}
	...
	public static function getCategoryId( $link_id, $cat_id ) {
		$db = JFactory::getDBO();
