...
class modMTCategoriesExpandingHelper {
	...
	static function getCategoryId($cat_id, $link_id) {
		$db = JFactory::getDBO();
	...
	static function getCategories( $params, $cat_id ) {
		global $mtconf;
		...
		$db->setQuery( $sql );
		$top_level_cats = $db->loadObjectList('cat_id');

		// Filter unauthorised categories
		$cats[0] = MTModuleHelper::filterUnauthorisedCategories( $top_level_cats );
