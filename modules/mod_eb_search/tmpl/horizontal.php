...
<form method="post" name="eb_search_form" id="eb_search_form" action="<?php echo JRoute::_('index.php?option=com_eventbooking&task=search&&Itemid='.$itemId);  ?>">
    <div class="filters <?php echo $btnToolbarClass; ?> eb-search-bar-container clearfix">
        <div class="filter-search <?php echo $btnGroupClass . ' ' . $pullLeftClass;;?>">
            <input name="search" id="search_eb_box" maxlength="50"  class="inputbox" type="text" size="20" value="<?php echo $text; ?>" placeholder="<?php echo JText::_('EB_SEARCH_WORD'); ?>" />
        </div>
        <?php
            if ($showFromDate)
            {
            ?>
                <div class="<?php echo $btnGroupClass . ' ' . $pullLeftClass; ?>">
	                <?php echo JHtml::_('calendar', $fromDate, 'filter_from_date', 'filter_from_date', $datePickerFormat, ['class' => 'input-medium', 'placeholder' => JText::_('EB_SEARCH_FROM_DATE')]); ?>
                </div>
            <?php
            }
            ...
            ?>
                <div class="<?php echo $btnGroupClass . ' ' . $pullLeftClass; ?>">
                    <?php echo JHtml::_('calendar', $toDate, 'filter_to_date', 'filter_to_date', $datePickerFormat, ['class' => 'input-medium', 'placeholder' => JText::_('EB_SEARCH_TO_DATE')]); ?>
                </div>
            <?php
            }
        ?>
