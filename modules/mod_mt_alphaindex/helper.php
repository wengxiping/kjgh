...
class modMTAlphaindexHelper {
	...
	public static function getList($params) {
		...
		// Count alpha_index_additional_chars
		if($mtconf->get('alpha_index_additional_chars') != '') {
			$alpha_index_additional_chars_length = StringHelper::strlen($mtconf->get('alpha_index_additional_chars'));

			for ( $i=0; $i < $alpha_index_additional_chars_length; $i++ )
			{

				$additional_char = StringHelper::substr($mtconf->get('alpha_index_additional_chars'), $i, 1);

				$lists[$additional_char] = new stdClass();
				$lists[$additional_char]->text = $additional_char;
				$lists[$additional_char]->link = JRoute::_( 'index.php?option=com_mtree&task=listalpha&alpha=' . strtolower($additional_char) . $itemid . $url_cat_id );
				$lists[$additional_char]->current = ($additional_char == strtolower($current_char)) ? true : false;
				...
				if( $display_total_links || !$show_empty )
				{
					...
					$where[] = "( publish_up = ".$db->Quote($nullDate)." OR publish_up <= '$now'  )";
					$where[] = "( publish_down = ".$db->Quote($nullDate)." OR publish_down >= '$now' )";
					$where[] = "link_name LIKE '".$additional_char."%'";
					$sql .= (count( $where ) ? ' WHERE ' . implode( ' AND ', $where ) : "");
					$sql .= (count( $where_cat ) ? ' AND ' . implode( ' AND ', $where_cat ) : "");

					$db->setQuery( $sql );
					$lists[$additional_char]->total = $db->loadResult();
				} else {
					$lists[$additional_char]->total = 0;
				}
			}
		}
