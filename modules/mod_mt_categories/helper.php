...
class modMTCategoriesHelper {
	...
	static function getCategories( $params, $cat_id, $link_id ) {
		...
		foreach( $cats AS $cat )
		{
			$cats[$cat->cat_id]->link = JRoute::_( 'index.php?option=com_mtree&task=listcats&cat_id=' . $cat->cat_id . $itemid );
			if( $cat_id > 0 && $cat_id == $cat->cat_id ) {
				$cats[$cat->cat_id]->active = true;
			} else {
				$cats[$cat->cat_id]->active = false;
			}
		}

		// Filter unauthorised categories
		$cats = MTModuleHelper::filterUnauthorisedCategories( $cats );

		return $cats;
	}

	static function getBackCategory( $params, $cat_id, $link_id, $current_category_is_empty ) {
		$db = JFactory::getDBO();
		$itemid = MTModuleHelper::getItemid();
	...
	static function getCategoryId( $link_id, $cat_id ) {
		$db = JFactory::getDBO();
	...
	static function getCategoryParentId( $cat_id ) {
		$db = JFactory::getDBO();
	...
	static function loadCategoriesList( $params, $cat_id ) {
		global $mtconf;
