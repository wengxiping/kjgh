 ...
 */
class Easysocial_Invitex_ProfileModel extends EasySocialModel
{
	/**
	 * Model for invitex app
	 *
	 * @param   STRING  $profile_name  User's id.
	 * @param   STRING  $profile_link  User's id.
	 *
	 * @return  Array  A list of textbook rowset.
	 *
	 * @since   1.0
	 */
	public function getInvitexView_profile($profile_name, $profile_link )
	{
		...
		$invite_typeid = $model->getInvite_typeID('ESProfile');

		// SET Variables input values
		$invite_url = $profile_link;

		JFactory::getApplication()->input->set('invite_url', urlencode($profile_link));
		JFactory::getApplication()->input->set('invite_type', $invite_typeid);
		JFactory::getApplication()->input->set('invite_anywhere', 1);
		JFactory::getApplication()->input->set('tag', '[name=' . $profile_name . ']');
