...
class SocialUserAppArticleHookNotificationLikes extends SocialAppHooks
{
	public function execute(&$item)
	{
		...
		if ($item->actor_id == $owner && count($users) == 1) {
			$item->title = JText::sprintf($this->getGenderForLanguage('APP_USER_ARTICLE_OWNER_LIKES_ITEM', $owner), $names);

			return $item;
		}

		$item->title = JText::sprintf($this->getPlurality('APP_USER_ARTICLE_USER_LIKES_USER_ITEM', $users), $names, ES::user($owner)->getName());
