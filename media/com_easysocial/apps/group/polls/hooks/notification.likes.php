...
class SocialGroupAppPollsHookNotificationLikes
{
	/**
	...
	public function execute(SocialTableNotification &$item)
	{
		...
		if ($item->context_type == 'polls.group.create') {
			...
			if ($stream->actor_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
				...
				$langString = ES::string()->computeNoun('APP_GROUP_POLLS_USERS_REACT_ON_YOUR_POLL', count($users));
			...
			$langString = ES::string()->computeNoun('APP_GROUP_POLLS_USER_REACT_ON_USERS_POLL', count($users));
			$item->title = JText::sprintf($langString, $names, $poll->title, $group->title);
