...
class SocialPageAppPollsHookNotificationLikes
{
	/**
	...
	public function execute(SocialTableNotification &$item)
	{
		...
		if ($item->context_type == 'polls.page.create') {
			...
			if ($stream->actor_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
				...
				$langString = ES::string()->computeNoun('APP_PAGE_POLLS_USERS_REACTED_YOUR_POLL', count($users));
			...
			$langString = ES::string()->computeNoun('APP_PAGE_POLLS_USER_POSTED_REACTED_USERS_POLL', count($users));
			$item->title = JText::sprintf($langString, $names, $poll->title, $page->title);
