 ...
 */
class Easysocial_Invitex_EventModel extends EasySocialModel
{
	/**
	 * Model for invitex app
	 *
	 * @param   STRING  $event_nm    User's id.
	 * @param   STRING  $event_link  User's id.
	 *
	 * @return  Array  A list of textbook rowset.
	 *
	 * @since   1.0
	 */
	public function getInvitexView($event_nm, $event_link)
	{
		...
		$invite_typeid = $model->getInvite_typeID('ESEvent');

		// SET Variables input values
		$invite_url = $event_link;

		JFactory::getApplication()->input->set('invite_url', urlencode($event_link));
		JFactory::getApplication()->input->set('invite_type', $invite_typeid);
		JFactory::getApplication()->input->set('invite_anywhere', 1);
		JFactory::getApplication()->input->set('tag', '[name=' . $event_nm . ']');
