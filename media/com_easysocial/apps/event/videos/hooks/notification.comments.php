	...
	public function execute(&$item)
	{
		...
		if ($item->context_type == 'videos.event.create') {
			...
			if ($video->user_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
				$item->title = JText::sprintf('APP_USER_VIDEOS_NOTIFICATIONS_COMMENTED_ON_YOUR_VIDEO', $names, $video->getTitle());
			...
			if ($item->actor_id == $video->user_id && count($users) == 1) {
				...
				$item->title = JText::sprintf('APP_USER_VIDEOS_NOTIFICATIONS_COMMENTED_ON_USERS_VIDEO' . FD::user($item->actor_id)->getGenderLang(), FD::user($item->actor_id)->getName(), $video->getTitle());
			...
			$item->title = JText::sprintf($string, $names, FD::user($video->user_id)->getName(), $video->getTitle());
