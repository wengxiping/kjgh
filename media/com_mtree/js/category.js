jQuery(document).ready(function() {
	jQuery('#mtfields select, #mtfields textarea').change(function() {
...
function updateMainCat(){
	...
	jQuery.getJSON(JURI_ROOT+"?option=com_mtree&task=fields.list&cat_id="+active_cat+"&link_id="+linkId+"&format=json"+(jQuery('input[name=is_admin]').val()?'&is_admin=1&tmpl=component':''), function(data) {
	  var items = [];
	...
	jQuery('#mtfields input, #mtfields select, #mtfields textarea').bind('change', function(event) {
...
function updateCachedFields() {
	//var fields = jQuery('#mtfields div[id^=field_] input').clone();
...
function cc(parent_cat_id){
	if(parent_cat_id >= 0 && parent_cat_id != '') {
		var jsonurl = '';
		if(jQuery('input[name=is_admin]').val()) {
			jsonurl = JURI_ROOT+"administrator/?option=com_mtree&task=ajax&task2=categories.list&cat_id="+parent_cat_id+"&format=json&is_admin=1&no_html=1&tmpl=component";
		} else {
			jsonurl = JURI_ROOT+"?option=com_mtree&task=ajax&task2=categories.list&cat_id="+parent_cat_id+"&format=json";
		}
