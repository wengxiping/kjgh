jQuery(document).ready(function(){
    ...
    enforceSecCatLimit();
    jQuery('#browsecat').change(function(){
        cc(jQuery(this).val());
    });
    ...
    jQuery('#mtForm .controls input, #mtForm .controls textarea, #mtForm .controls select').on({
...
function mtValidateNonCheckboxesRadios(target){
    ...
    if(typeof(jQuery('#'+id).attr('required'))=='undefined'){
        var attrRequired = jQuery('input#'+id).attr('required');
    } else {
        var attrRequired = jQuery('#'+id).attr('required');
    }
...
function mtValidateCheckboxesRadios(target){
    ...
    if(
        jQuery('#mtForm .controls input[required][name="'+jQuery('#'+target.id).attr('name')+'"]:checked').length==0
        ) {
        return false;
    } else {
        return true;
...
function getCfId(name){
    ...
    return parseInt(str.slice(2).split('_').shift());
}