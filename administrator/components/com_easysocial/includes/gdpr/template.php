...
class SocialGdprTemplate extends JObject
{
	public $id = null;
	public $type = null;
	public $created = null;
	 ...
	 * @access  public
	 */
	public function createViewFile(SocialGdrpTab $tab)
	{
	 ...
	 * @access  public
	 */
	public function getListingContent(SocialGdrpTab $tab)
	{
...
<?php
	 ...
	 * @access  public
	 */
	public function hasRelation(SocialGdrpTab $tab)
	{
		$path = false;

		if ($this->relation) {
			$userTmpPath = SocialGdpr::getUserTempPath($tab->adapter->user);
	...
	{
		$baseUrl = '';
		$sidebar = SocialGdpr::getSidebarContents(false, $tab->key);
		...
		if ($this->hasSource()) {
			...
			if (!is_array($sources)) {
				$sources = ES::makeArray($sources);
			}

			if (!is_array($sourceFilenames)) {
				$sourceFilenames = ES::makeArray($sourceFilenames);
			}
	...
	public function processMedia($tmpPath, $source, $fileName = '')
	{
		...
		ES::makeFolder($tmpPath . '/' . $folderPath);
