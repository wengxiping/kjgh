...
class FFmpeg
{
	...
	*/
	 ...
	 * @access  public
	 */
	public function resize($input, $output, $bitrate, $size, $logFile, $framerate, $debug = false)
	{
		$command = $this->ffmpeg . ' ';

		// -r 25 is output frame rate (in frames per second) to avoid some other frame unable to encode then it will show these warning message Past duration X.XXX too large
		$command .= '-y -i ' . $input . ' -r ' . $framerate . ' -strict -2 ';
