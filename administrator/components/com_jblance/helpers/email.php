...
class EmailHelper {
	...
	function sendRegistrationMail(&$usern, $password, $facebook=false){
		...
		$usertype = $usergroupInfo->ug_name;
	...
	function sendNewProjectNotification($project_id, $isNewProject){
		...
		$matchSkills   = $config->projectMatchSkills;
		$matchLocation = $config->projectMatchLocation;
		...
		if(!$isPvtInvite){
			...
			$canBidUserGroup = ArrayHelper::toInteger($canBidUserGroup);
			...
			if($matchSkills){
				...
				$id_categ = ArrayHelper::toInteger($id_categ);		//sanitize array
			...
			$invite_user_id = JblanceHelper::sanitizeCsvString($project->invite_user_id);
	...
	function sendNewBidNotification($bid_id, $project_id, $isNewBid){
		...
		$query = "SELECT notifyBidNewAcceptDeny FROM #__jblance_notify WHERE user_id=".$db->quote($project->publisher_userid);
		$db->setQuery($query);
	...
	function sendOutBidNotification($bid_id, $project_id){
		...
		$query = "SELECT email FROM #__jblance_bid b ".
				 ...
				 "WHERE b.amount > ".$db->quote($bid->amount)." ".
				 "AND  outbid = 1 AND project_id =".$db->quote($project_id);
		$db->setQuery($query);
		$bcc = $db->loadColumn();
	...
	function sendBidWonNotification($project_id){
		...
		$query = "SELECT notifyBidWon FROM #__jblance_notify WHERE user_id=".$db->quote($project->assigned_userid);
		$db->setQuery($query);
		$notify = $db->loadResult();
		...
		//get bid details
		$query = "SELECT amount,delivery FROM #__jblance_bid WHERE user_id=".$db->quote($project->assigned_userid)." AND project_id = ".$db->quote($project_id);
		$db->setQuery($query);
		$bid = $db->loadObject();
		$bidamount = JblanceHelper::formatCurrency($bid->amount, false).$perHr;
	...
	function sendProjectAcceptedNotification($project_id, $bidder_id){
		...
		$query = "SELECT email FROM #__jblance_bid b ".
				 ...
				 "WHERE project_id=".$db->quote($project_id)." AND user_id NOT IN (".(int)$project->assigned_userid.")";
		$db->setQuery($query);
	...
	function sendMessageNotification($msg_id){
		...
		$query = "SELECT notifyNewMessage FROM #__jblance_notify WHERE user_id=".$db->quote($recipient_id);
		$db->setQuery($query);
		$notify = $db->loadResult();
	...
	function sendExpiryEmail($email, $uname, $type, $subscr_project_id){
		...
		if($type == 'subscr'){
			...
			$tags = array("[SITENAME]", "[SITEURL]", "[PLANNAME]", "[NAME]", "[SUBSCREXPIRYDATE]");
			$tagsValues = array("$sitename", "$siteURL", "$plan->name", "$uname", "$expireDate");
			
			//get the email template
			$template = $this->getTemplate('subscr-expiry-reminder');
