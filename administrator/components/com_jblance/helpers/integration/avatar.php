...
abstract class JoombriAvatar {
	...
	abstract protected function _getURL($userid, $folder);

	public function getLink($userid, $att = '', $folder){
		$imgurl = $this->_getURL($userid, $folder);
		if(!$imgurl) return;
	...
	public function getLinkOnly($userid, $folder){
		$imgurl = $this->_getURL($userid, $folder);
		return $imgurl;
	}
}
