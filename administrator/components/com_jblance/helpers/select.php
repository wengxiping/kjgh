...
class SelectHelper {
	...
	function getSelectLocationTree($var, $default, $oldId, $title = 'JGLOBAL_ROOT_PARENT', $attribs, $event = ''){
		...
		$lists 	= JHtml::_('select.genericlist', $options, $var, "$attribs $event", 'value', 'text', $default);
		return $lists;
	}
	...
	function getNewSelectCategoryTree($var){
		...
		foreach($categs as $categ) {
			$html .="<div class='list-category-group'><div class='group-label'>".$categ->category."</div>";
			$subs = $this->getSubcategories($categ->id, "", '', 0);
			$html .="<div class='group-content'>";
			foreach($subs as $sub) {
				$category = str_replace("-","",$sub->category);
				$html .="<div class='item' data-category-type=1 data-category-txt='".$category."' data-category-id=".$sub->id."><div class='txt'>".$category."</div><div class='choose-img'></div></div>";
	...
	function getSelectCategoryTree($var, $default, $title, $attribs, $event = '', $group = false, $rootOnly = false){
		...
		//if attribs is empty, then set a default one.
		if(empty($attribs))
			$attribs = "class='input-medium' size='10'";
	...
	function getCheckCategoryTree($var, $default, $attribs){
		...
		foreach($categs as $categ) {
			...
			$subs = $this->getSubcategories($categ->id, $indent, '', 0);
			echo '<div class="row-fluid" style="padding: 0 15px;"><div class="span12">';
			foreach($subs as $sub) {
	...
	function getSubcategories($parent, $indent, $init, $type = 1, $showUnpublished = false){
		$db = JFactory::getDbo();
		...
		$unpublished = 'published=1';
		if($showUnpublished){
			$unpublished = 'TRUE';
		}
	
		$query = "SELECT * FROM #__jblance_category WHERE parent =".$db->quote($parent)." AND ".$unpublished." ORDER BY ordering";
		$db->setQuery($query);
	...
	function getSelectRating($var, $default){
		$put[] = JHtml::_('select.option',  "");
		$put[] = JHtml::_('select.option',  1, JText::_('COM_JBLANCE_VERY_POOR'));
		$put[] = JHtml::_('select.option',  2, JText::_('COM_JBLANCE_POOR'));
		$put[] = JHtml::_('select.option',  3, JText::_('COM_JBLANCE_ACCEPTABLE'));
		$put[] = JHtml::_('select.option',  4, JText::_('COM_JBLANCE_GOOD'));
		$put[] = JHtml::_('select.option',  5, JText::_('COM_JBLANCE_EXCELLENT'));
