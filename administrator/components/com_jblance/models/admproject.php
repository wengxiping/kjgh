...
class JblanceModelAdmproject extends JModelLegacy {
 	...
 	function getDashboard(){
 		...
 		// total no of Joomla users
 		$query = "SELECT COUNT(*) FROM #__users";
 		$db->setQuery($query);
 		$users = $db->loadResult();
 	
 		// total no of JoomBri users
 		...
 		$query = "SELECT COUNT(*) FROM #__jblance_service";
 		$db->setQuery($query);
 		$services = $db->loadResult();
 		
 		// total no of Unapproved Projects
 		$query = "SELECT COUNT(*) FROM #__jblance_project WHERE approved=0";
 		$db->setQuery($query);
 		$unappProjs = $db->loadResult();
 		
 		// total no of Unapproved Services
 		$query = "SELECT COUNT(*) FROM #__jblance_service WHERE approved=0";
 		$db->setQuery($query);
 		$unappSvcs = $db->loadResult();
 		
 		// total no of Unapproved Users
 		$query = "SELECT COUNT(*) FROM #__users WHERE block=1";
 		$db->setQuery($query);
 		$unappUsers = $db->loadResult();
 		
 		// total no of Unapproved Subscriptions
 		$query = "SELECT COUNT(*) FROM #__jblance_plan_subscr WHERE approved=0";
 		$db->setQuery($query);
 		$unappSubs = $db->loadResult();
 		
 		// total no of Unapproved Deposits
 		$query = "SELECT COUNT(*) FROM #__jblance_deposit WHERE approved=0";
 		$db->setQuery( $query );
 		$unappDepo = $db->loadResult();
 		
 		// total no of Unapproved Withdraws
 		$query = "SELECT COUNT(*) FROM #__jblance_withdraw WHERE approved=0";
 		$db->setQuery( $query );
 		$unappWdraws = $db->loadResult();
 		
 		// total no of Unapproved Messages
 		$unappMsgs = JblanceHelper::countUnapprovedMsg();
 		
 		//version installed
 		$filename 	 = JPATH_COMPONENT_ADMINISTRATOR.'/jblance.xml';
 		$xmlManifest = simplexml_load_file($filename);
 		
 		//get the current version from server
 		$updateServerUrl = "http://www.joombri.in/updates/jblance-update.xml";
 		$response = JblanceHelper::getHttpResponse($updateServerUrl);
 		...
 		if(($response === false) || ($response->code != 200)){
			...
			if(is_object($xml)) {
				foreach ($xml->children() as $package) {
					...
					$ext->platform = (string)$package->targetplatform->attributes()->version;
					$ext->tag = (string)$package->tags->tag;
					
					if(version_compare(JVERSION, '1.6.0', 'ge')  && version_compare(JVERSION, '3.0.0', 'lt')) {
						//current version is J2.5. Get the version available for 2.5
						if($ext->platform == '2.5'){
					...
					elseif(version_compare(JVERSION, '3.0.0', 'ge')){
						if($ext->platform == '3.' || $ext->platform == '3.5'){
							$newVersion = $ext->version;
							$tag = $ext->tag;
						}
					}
 		...
 		$return[5] = $unappSubs;
 		$return[6] = $unappDepo;
 		$return[7] = $unappWdraws;
 		$return[8] = $unappMsgs;
 		$return[9] = $xmlManifest;
 		$return[10] = $newVersion;
 		$return[11] = $services;
 		$return[12] = $unappSvcs;
 		$return[13] = $tag;
 		return $return;
 	...
 	function getShowProject(){
		...
		$filter_status	  = $app->getUserStateFromRequest('com_jblance_filter_status', 'filter_status', 'any', 'string');
		...
		$lists['status'] 	= $select->getSelectProjectStatus('filter_status', $filter_status, 'COM_JBLANCE_ALL_STATUS', '', 'onchange="document.adminForm.submit();"');
		$lists['search'] 	= $search;
 	...
 	function getEditProject(){
 		...
 		$lists['userlist'] 	= JHtml::_('select.genericlist', $types, 'publisher_userid', 'class="input-xlarge required advancedSelect" size="1"', 'value', 'text', $row->publisher_userid ? $row->publisher_userid : '0');
 		
 		//get project files and bid info
 		$projfiles = $bids = array();
 		if(!$isNew){
	 		$query = "SELECT * FROM #__jblance_project_file WHERE project_id=".$db->quote($cid[0]);
	 		$db->setQuery($query);
	 		$projfiles = $db->loadObjectList();
	 		
	 		$query = "SELECT b.*, u.username FROM #__jblance_bid b ". 
	 				 "INNER JOIN #__users u ON b.user_id=u.id ".
	 				 "WHERE b.project_id =".$db->quote($cid[0]);
	 		$db->setQuery($query);
	 		$bids = $db->loadObjectList();
 		}
 		
 		$query = "SELECT * FROM #__jblance_custom_field ".
 				 "WHERE published=1 AND field_for='project' ".
 				 "ORDER BY ordering";
 		$db->setQuery($query);
 	...
 	function getShowService(){
 		$app = JFactory::getApplication();
 		$db	= JFactory::getDbo();
 		$where = array();
 		
 		$filter_order     = $app->getUserStateFromRequest('com_jblance_filter_order_service', 'filter_order', 's.service_title', 'cmd');
		$filter_order_Dir = $app->getUserStateFromRequest('com_jblance_filter_order_Dir_service', 'filter_order_Dir', 'asc', 'word');
		$search			  = $app->getUserStateFromRequest('com_jblance_service_search', 'search', '', 'string');
		if(strpos($search, '"') !== false){
			$search = str_replace(array('=', '<'), '', $search);
		}
		$search = strtolower($search);
		...
		if(isset($search) && $search != ''){
			$searchEscaped = $db->quote( '%'.$db->escape( $search, true ).'%', false );
			$where[] = "s.service_title LIKE ".$searchEscaped;
		}
 	...
 	function getEditService(){
 		...
 		$lists['userlist'] 	= JHtml::_('select.genericlist', $types, 'user_id', 'class="input-xlarge required" size="1"', 'value', 'text', $row->user_id ? $row->user_id : '0');
 		
 		//get list of orders
 		$orders = array();
 		if(!$isNew){
 			$query ="SELECT so.*, u.username FROM #__jblance_service_order so ".
 					"INNER JOIN #__users u ON so.user_id=u.id ".
 					"WHERE so.service_id =".$db->quote($cid[0]);
 			$db->setQuery($query);
 	...
 	function getShowUser(){
 		...
 		if($ug_id == -1){
 			$where[] = "ju.ug_id IS NULL";
 		}
 		if($ug_id > 0){
 			$where[] = "ju.ug_id=".$db->quote($ug_id);
 		}
 	...
 	function getEditUser(){
 		...
 		$query = "SELECT ju.*, u.name FROM #__users u ".
 				 "LEFT JOIN #__jblance_user ju ON ju.user_id = u.id ".
 				 "WHERE u.id=".$db->quote($cid[0]);
 		$db->setQuery($query);	//echo $query;
 		$row = $db->loadObject();
 		...
 		$lists 	= JHtml::_('select.genericlist', $types, 'username', 'class="input-xlarge required advancedSelect" size="1" '.$disabled.'', 'value', 'text', /*$row->user_id ? $row->user_id : */$cid[0]);
 		...
 		$grpLists 	= JHtml::_('select.genericlist', $types, 'ug_id', "class='input-medium required advancedSelect' size='1' $disabled $event", 'value', 'text', $row->ug_id ? $row->ug_id : $post_ugid);
 	
 		//get the transaction list of the user
 		$query = "SELECT * FROM #__jblance_transaction ".
 				 "WHERE user_id =".$db->quote($row->user_id)." ".
 	...
 	function getShowSubscr(){
 		...
 		if($user_id > 0)	 $where[] = "s.user_id=".$db->quote($user_id);
 		if($subscr_id > 0) 	 $where[] = "s.id=".$db->quote($subscr_id);
 		if($subscr_plan > 0) $where[] = "s.plan_id=".$db->quote($subscr_plan);
 		if($ug_id != '') 	 $where[] = "p.ug_id=".$db->quote($ug_id);
 		if($subscr_status != ''){
 			if($subscr_status == 3)
 				$where[] = '(TO_DAYS(s.date_expire) - TO_DAYS(NOW())) < 0';
 			else
 				$where[] = "s.approved=".$db->quote($subscr_status);
 		}
 	...
 	function getEditSubscr(){
 		...
 		$attribs = "class='input-large required' size='1' $disabled";
 		...
 		$types[] = JHtml::_('select.option', '0', JText::_('COM_JBLANCE_PENDING'));
 		...
 		$lists['status'] = JHtml::_('select.genericlist', $types, 'approved', "class='input-large required' size='1'", 'value', 'text', $row->approved);
 	
 		$user_where = $plan_where = 'true';
 		if(!empty($ug_id)){
 			$user_where = " ju.ug_id = ".$db->quote($ug_id);
 			$plan_where = " p.ug_id = ".$db->quote($ug_id);
 		}
 		...
 		$users 	= JHtml::_('select.genericlist', $types, 'user_id', 'class="input-large required" size="1" '.$disabled.'', 'value', 'text', $row->user_id ? $row->user_id : '0');
 	
 		//make plans selection list
 		$query = "SELECT * FROM #__jblance_plan p ".
 				 "WHERE ".$plan_where." AND p.published=1 ".
 				 "ORDER BY p.id ASC";
 		$db->setQuery($query);
 		...
 		$plans 	= JHtml::_('select.genericlist', $types, 'plan_id', 'class="input-large required" size="1" '.$disabled.'', 'value', 'text', $row->plan_id ? $row->plan_id : '0');
 	...
 	function getShowDeposit(){
 		...
 		if($user_id > 0) 	$where[] = "d.user_id=".$db->quote($user_id);
 		if($credit_id > 0) 	$where[] = "d.id=".$db->quote($credit_id);
 	...
 	function getShowWithdraw(){
 		...
 		if($user_id > 0) 	$where[] = "w.user_id=".$db->quote($user_id);
 		if($credit_id > 0) 	$where[] = "w.id=".$db->quote($credit_id);
 	...
 	function getShowsEscrow(){
 		...
 		$query = "SELECT e.*,s.name sender,r.name receiver,COALESCE(p.project_title, sv.service_title) title FROM #__jblance_escrow e ".
				 "LEFT JOIN #__users s ON e.from_id = s.id ".
				 "LEFT JOIN #__users r ON e.to_id = r.id ".
				 "LEFT JOIN #__jblance_project p ON (e.project_id = p.id AND e.type = 'COM_JBLANCE_PROJECT') ".
				 "LEFT JOIN #__jblance_service_order so ON (so.id=e.project_id AND e.type = 'COM_JBLANCE_SERVICE') ".
				 "LEFT JOIN #__jblance_service sv ON (sv.id=so.service_id) ".
 	...
 	function getShowSummary(){
 		...
 		$lists['search_month'] = JHtml::_('select.genericlist', $types, 'search_month', "class='input-small' size='1' $event", 'value', 'text', $search_month);
 		...
 		$lists['search_year'] = JHtml::_('select.genericlist', $types, 'search_year', "class='input-small' size='1' $event", 'value', 'text', $search_year);
 		
 		if($search_month > 0){
 			$where_d[]  = "MONTH(date_deposit)=".$db->quote($search_month)." AND YEAR(date_deposit)=".$db->quote($search_year);
 			$where_w[]  = "MONTH(date_withdraw)=".$db->quote($search_month)." AND YEAR(date_withdraw)=".$db->quote($search_year);
 			$where_p[]  = "MONTH(create_date)=".$db->quote($search_month)." AND YEAR(create_date)=".$db->quote($search_year);
 			$where_b[]  = "MONTH(bid_date)=".$db->quote($search_month)." AND YEAR(bid_date)=".$db->quote($search_year);
 			$where_s[]  = "MONTH(date_buy)=".$db->quote($search_month)." AND YEAR(date_buy)=".$db->quote($search_year);
 			$where_sv[] = "MONTH(create_date)=".$db->quote($search_month)." AND YEAR(create_date)=".$db->quote($search_year);
 			$where_svo[] = "MONTH(order_date)=".$db->quote($search_month)." AND YEAR(order_date)=".$db->quote($search_year);
 		}
 		else {
 			$where_d[]  = "YEAR(date_deposit)=".$db->quote($search_year);
 			$where_w[]  = "YEAR(date_withdraw)=".$db->quote($search_year);
 			$where_p[]  = "YEAR(create_date)=".$db->quote($search_year);
 			$where_b[]  = "YEAR(bid_date)=".$db->quote($search_year);
 			$where_s[]  = "YEAR(date_buy)=".$db->quote($search_year);
 			$where_sv[] = "YEAR(create_date)=".$db->quote($search_year);
 			$where_svo[] = "YEAR(order_date)=".$db->quote($search_year);
 		}
 		...
 		$where_b  = (count( $where_b) ? ' (' . implode( ') AND (', $where_b ) . ')' : '' );
 		$where_s  = (count( $where_s) ? ' (' . implode( ') AND (', $where_s ) . ')' : '' );
 		$where_sv = (count( $where_sv) ? ' (' . implode( ') AND (', $where_sv ) . ')' : '' );
 		$where_svo = (count( $where_svo) ? ' (' . implode( ') AND (', $where_svo ) . ')' : '' );
 		
 		$query = "SELECT gateway, SUM(total-amount) AS profit FROM #__jblance_deposit ".
 				 "WHERE approved=1 AND ".$where_d." ".
 				 "GROUP BY gateway";
 		$db->setQuery($query);
 		$deposits = $db->loadObjectList();
 		
 		$query = "SELECT gateway, SUM(amount-finalAmount) AS profit FROM #__jblance_withdraw ".
 				 "WHERE approved=1 AND ".$where_w." ".
 				 "GROUP BY gateway";
 		$db->setQuery($query);
 		$withdraws = $db->loadObjectList();
 		
 		$query = "SELECT SUM(profit) AS profit_comm, SUM(profit_additional) AS profit_upgrades, SUM(profit_perproject) AS profit_perproject FROM #__jblance_project ".
 				 "WHERE ".$where_p;
 		$db->setQuery($query);
 		$project = $db->loadObject();
 		
 		$query = "SELECT SUM(profit_perbid) AS profit_perbid FROM #__jblance_bid ".
 				 "WHERE ".$where_b;
 		$db->setQuery($query);
 		$project->profit_perbid = $db->loadResult();
 		...
 		$query = "SELECT SUM(profit_perservice) AS profit_service FROM #__jblance_service s ".
 				 "WHERE ".$where_sv;//echo $query;
 		$db->setQuery($query);
 		$service = $db->loadObject();
 		
 		$query = "SELECT SUM(so.price-e.amount) AS profit_serviceorder FROM #__jblance_service_order so ".
				 "JOIN #__jblance_escrow e ON so.id=e.project_id AND e.type='COM_JBLANCE_SERVICE' ".
 				 "WHERE ".$where_svo;//echo $query;
 		$db->setQuery($query);
 		$service->profit_serviceorder = $db->loadResult();
 		
 		//get the chart data
 		$db	= JFactory::getDbo();
 		$query = "SELECT MONTH(date_buy) Month, SUM(price) AS profit_subscr FROM #__jblance_plan_subscr ".
 	 			 "WHERE approved=1 AND YEAR(date_buy)=".$db->quote($search_year)." ".
 				 "GROUP BY MONTH(date_buy)";
 		$db->setQuery($query);
 		$chart_subscr = $db->loadObjectList();
 		
 		$query = "SELECT MONTH(date_deposit) Month, SUM(total-amount) AS profit_deposit FROM #__jblance_deposit ".
 	 			 "WHERE approved=1 AND YEAR(date_deposit)=".$db->quote($search_year)." ".
 				 "GROUP BY MONTH(date_deposit)";
 		$db->setQuery($query);
 		$chart_deposit = $db->loadObjectList();
 		
 		$query = "SELECT MONTH(date_withdraw) Month, SUM(amount-finalAmount) AS profit_withdraw FROM #__jblance_withdraw ".
 	 			 "WHERE approved=1 AND YEAR(date_withdraw)=".$db->quote($search_year)." ".
 				 "GROUP BY MONTH(date_withdraw)";
 		$db->setQuery($query);
 		$chart_withdraw = $db->loadObjectList();
 		
 		$query = "SELECT MONTH(create_date) Month, SUM(profit+profit_additional+profit_perproject) AS profit FROM #__jblance_project ".
 	 			 "WHERE YEAR(create_date)=".$db->quote($search_year)." ".
				 "GROUP BY MONTH(create_date)";
 		$db->setQuery($query);
 		$chart_project = $db->loadObjectList();
 			
 		$query = "SELECT MONTH(bid_date) Month, SUM(profit_perbid) AS profit FROM #__jblance_bid ".
 	 			 "WHERE YEAR(bid_date)=".$db->quote($search_year)." ".
				 "GROUP BY MONTH(bid_date)";
 		$db->setQuery($query);
 		$chart_bid = $db->loadObjectList();
 		
 		$query = "SELECT MONTH(create_date) Month, SUM(profit_perservice) AS profit_service FROM #__jblance_service s ".
 				 "WHERE YEAR(create_date)=".$db->quote($search_year)." ".
				 "GROUP BY MONTH(create_date)";
 		$db->setQuery($query);
 		$chart_service = $db->loadObjectList();
 		
		$query = "SELECT MONTH(order_date) Month, SUM(so.price-e.amount) AS profit_service FROM #__jblance_service_order so ".
 				"JOIN #__jblance_escrow e ON so.id=e.project_id AND e.type='COM_JBLANCE_SERVICE' ".
 				"WHERE YEAR(order_date)=".$db->quote($search_year)." ".
 				"GROUP BY MONTH(order_date)";
 		$db->setQuery($query);
 		$chart_serviceorder = $db->loadObjectList();
 		
 		//process the above data for 1-12 months
 		for($i=1; $i<=12; $i++){
 			$new_deposit[$i] = new stdClass();
 			$new_subscr[$i]  = new stdClass();
 			$new_withdrw[$i] = new stdClass();
 			$new_project[$i] = new stdClass();
 			$new_service[$i] = new stdClass();
 			
 			$new_subscr[$i]->Month = $i;
 			$new_subscr[$i]->profit_subscr = 0;
 			if(!empty($chart_subscr)){
 				foreach($chart_subscr as $s){
 					if($s->Month == $i){
 						$new_subscr[$i]->profit_subscr += $s->profit_subscr;
 					}
 				}
 			}
 			
 			$new_deposit[$i]->Month = $i;
 			$new_deposit[$i]->profit_deposit = 0;
 			if(!empty($chart_deposit)){
 				foreach($chart_deposit as $d){
 					if($d->Month == $i){
 						$new_deposit[$i]->Month = $d->Month;
 						$new_deposit[$i]->profit_deposit += $d->profit_deposit;
 					}
 				}
 			}
 			
 			$new_withdrw[$i]->Month = $i;
 			$new_withdrw[$i]->profit_withdraw = 0;
 			if(!empty($chart_withdraw)){
 				foreach($chart_withdraw as $w){
 					if($w->Month == $i){
 						$new_withdrw[$i]->Month = $w->Month;
 						$new_withdrw[$i]->profit_withdraw += $w->profit_withdraw;
 					}
 				}
 			} 
 			
 			$new_project[$i]->Month = $i;
 			$new_project[$i]->profit = 0;
 			if(!empty($chart_project)){
 				foreach($chart_project as $p){
 					if($p->Month == $i){
 						$new_project[$i]->profit += $p->profit;
 						break;
 					}
 				}
 			} 
 			if(!empty($chart_bid)){
 				foreach($chart_bid as $b){
 					if($b->Month == $i){
 						$new_project[$i]->profit += $b->profit;
 					}
 				}
 			}
 			
 			$new_service[$i]->Month = $i;
 			$new_service[$i]->profit_service = 0;
 			if(!empty($chart_service)){
 				foreach($chart_service as $sv){
 					if($sv->Month == $i){
 						$new_service[$i]->Month = $sv->Month;
 						$new_service[$i]->profit_service += $sv->profit_service;
 					}
 				}
 			}
 			if(!empty($chart_serviceorder)){
 				foreach($chart_serviceorder as $svo){
 					if($svo->Month == $i){
 						$new_service[$i]->Month = $svo->Month;
 						$new_service[$i]->profit_service += $svo->profit_service;
 					}
 				}
 			}
 		...
 		*/
			array('label' => JText::_('COM_JBLANCE_MONTH'), 'type' => 'string'),
			array('label' => JText::_('COM_JBLANCE_TITLE_SUBSCRIPTIONS'), 'type' => 'number'),
 			array('label' => JText::_('COM_JBLANCE_TITLE_DEPOSITS'), 'type' => 'number'),
 			array('label' => JText::_('COM_JBLANCE_TITLE_WITHDRAWALS'), 'type' => 'number'),
 			array('label' => JText::_('COM_JBLANCE_TITLE_PROJECTS'), 'type' => 'number'),
 			array('label' => JText::_('COM_JBLANCE_TITLE_SERVICES'), 'type' => 'number')
 		);
 		...
 		for($i=1; $i<=12; $i++){
 			...
 			$temp[] = array('v' => $s->Month, 'f' => $date->monthToString($s->Month));
 			$temp[] = array('v' => $s->profit_subscr, 'f' => JblanceHelper::formatCurrency($s->profit_subscr));
 			$temp[] = array('v' => $d->profit_deposit, 'f' => JblanceHelper::formatCurrency($d->profit_deposit));
 			$temp[] = array('v' => $w->profit_withdraw, 'f' => JblanceHelper::formatCurrency($w->profit_withdraw));
 			$temp[] = array('v' => $p->profit, 'f' => JblanceHelper::formatCurrency($p->profit));
 			$temp[] = array('v' => $sv->profit_service, 'f' => JblanceHelper::formatCurrency($sv->profit_service));
 	...
 	function getDetailReporting(){
 		...
 		$query = "SELECT * FROM #__jblance_report_reporter rr ".
 				 "WHERE rr.report_id=".$db->quote($cid[0])." ".
 				 "ORDER BY date_created DESC";//echo $query;
 		$db->setQuery($query);
 		$db->execute();
 		$total = $db->getNumRows();
 	...
 	function getManageMessage(){
 		...
 		if(!empty($cid[0])){
 			$query = "SELECT * FROM #__jblance_message ".
 					 "WHERE (id=".$db->quote($cid[0])." OR parent=".$db->quote($cid[0]).") AND deleted=0 ".
 	...
 	function getInvoice(){
 		...
 		$invoiceTitle = JText::_('COM_JBLANCE_INVOICE_NO');
 		
 		if($type == 'project')
 			$invoiceTitle = JText::_('COM_JBLANCE_PROJECTID');
 		elseif($type == 'service')
 			$invoiceTitle = JText::_('COM_JBLANCE_SERVICE_ORDER_ID');
 		
 		$row->invoiceTitle = $invoiceTitle;
 	...
 	function getSelectSubscrStatus($var, $default, $disabled, $event){
 		...
 		$types[] = JHtml::_('select.option', '', '- '.JText::_('COM_JBLANCE_SELECT_SUBSCR_STATUS').' -');
 		$types[] = JHtml::_('select.option', '0', JText::_('COM_JBLANCE_PENDING'));
 		$types[] = JHtml::_('select.option', '1', JText::_('COM_JBLANCE_COMPLETED'));
 		$types[] = JHtml::_('select.option', '2', JText::_('COM_JBLANCE_CANCELLED'));
 		$types[] = JHtml::_('select.option', '3', JText::_('COM_JBLANCE_EXPIRED'));
 	
 		$lists 	 = JHtml::_('select.genericlist', $types, $var, "class=\"input-large\" size=\"1\" $option $event", 'value', 'text', $default );
 	...
 	function getSelectPlan($var, $default, $disabled, $ug_id, $event){
 		...
 		$where = '';
 		if($ug_id)
 			$where = "p.ug_id=".$db->quote($ug_id)." AND";
 	
 		//make selection plans
 		$query = "SELECT id as value, name as text FROM #__jblance_plan p ".
	...
	function getSelectPaymode($var, $default, $event){
		...
		$lists 	= JHtml::_('select.genericlist', $types, $var, 'class="input-large" size="1" '.$event.'', 'value', 'text', $default);
		return $lists;
	}
	...
	function getSelectDepositStatus($var, $default, $event){
 	...
 	function countBids($id){
 		$db = JFactory::getDbo();
 		$query = "SELECT COUNT(*) FROM #__jblance_bid WHERE project_id=".$db->quote($id);
 		$db->setQuery($query);
 		$total = $db->loadResult();
 		return $total;
 	}
