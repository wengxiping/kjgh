 ...
 */
	function newProject(){
		...
		$link	= 'index.php?option=com_jblance&view=admproject&layout=editproject';
	...
	function saveProject(){
		...
		$id_category = ArrayHelper::toInteger($id_category);
		...
		//build and save the commitment value
		$commitment	= $app->input->get('commitment', null, 'array');
		$clean = JblanceHelper::cleanParams($commitment);
		$registry = new JRegistry();
		$registry->loadArray($clean);
		$post['commitment'] = $registry->toString();
		
		//process project image files
		$projectImageFiles	= $app->input->get('projectImage', null, 'array');
		$clean = JblanceHelper::cleanParams($projectImageFiles);
		$registry = new JRegistry();
		$registry->loadArray($clean);
		$post['project_image'] = $registry->toString();
 ...
 */
	function newService(){
		...
		$link	= 'index.php?option=com_jblance&view=admproject&layout=editservice';
		$this->setRedirect($link);
	}
	...
	function saveService(){
		...
		//process extra add-ons; since extras is a multi-dimensional array, loop it to clean it
		$extras	= $app->input->get('extras', null, 'array');
		foreach($extras as $k1 => $v1){
			$clean[$k1] = JblanceHelper::cleanParams($v1);
		}
		$registry = new JRegistry();
		$registry->loadArray($clean);
		$post['extras'] = $registry->toString();
		
 		//process service image files
 		$serviceImageFiles	= $app->input->get('serviceImage', null, 'array');
 		$clean = JblanceHelper::cleanParams($serviceImageFiles);
 		$registry = new JRegistry();
 		$registry->loadArray($clean);
 ...
 */	
	function saveUser(){
		...
		$name = $post['name'];
		$query = "UPDATE #__users SET name=".$db->quote($name)." WHERE id=".$db->quote($user_id);
		$db->setQuery($query);
		$db->execute();
	...
	function newSubscr(){
		...
		$link	= 'index.php?option=com_jblance&view=admproject&layout=editsubscr';
		$this->setRedirect($link);
	}
	...
	function removeSubscr(){
		...
		if(count($cid)){
			for($i = 0; $i < count($cid); $i++){
				$query = "DELETE FROM #__jblance_plan_subscr WHERE id=".$db->quote($cid[$i]);
				$db->setQuery($query);
				if(!$db->execute()){
					JError::raiseError($db->getErrorNum(), $db->getErrorMsg());
				}
	...
	function saveSubscr(){
		...
		if($isNew){
			//calculate the price
			$query = "SELECT id, days, days_type, price, discount, bonusFund, name FROM #__jblance_plan WHERE id=".$db->quote($row->plan_id);
			$db->setQuery($query);
			$plan = $db->loadObject();
	
			if($plan->discount){
				$query = "SELECT COUNT(*) AS total FROM #__jblance_plan_subscr WHERE plan_id=".$db->quote($row->plan_id)." AND user_id=".$db->quote($row->user_id);
				$db->setQuery($query);
				$total = $db->loadResult();
				if($total > 0){
 ...
 */
	...
	function removeDeposit(){
		...
		if(count($cid)){
			for($i = 0; $i < count($cid); $i++){
				$query = "DELETE FROM #__jblance_deposit WHERE id=".$db->quote($cid[$i]);
				$db->setQuery($query);
				$delCount++;
				if(!$db->execute()){
					JError::raiseError($db->getErrorNum(), $db->getErrorMsg());
 ...
 */
	...
	function removeWithdraw(){
		...
		if(count($cid)){
			for($i = 0; $i < count($cid); $i++){
				$query = "DELETE FROM #__jblance_withdraw WHERE id=".$db->quote($cid[$i]);
				$db->setQuery($query);
				$delCount++;
				if(!$db->execute()){
					JError::raiseError($db->getErrorNum(), $db->getErrorMsg());
 ...
 */
	...
	function removeEscrow(){
		...
		$cids = implode(',', $cid);
		$query = "DELETE FROM #__jblance_escrow WHERE id IN (".$cids.")";
		$db->setQuery($query);
		if(!$db->execute()){
			JError::raiseError($db->getErrorNum(), $db->getErrorMsg());
		}
		...
		$msg	= $delCount.' '.JText::_('COM_JBLANCE_ESCROW_DELETED_SUCCESSFULLY');
		$link	= 'index.php?option=com_jblance&view=admproject&layout=showescrow';
		$this->setRedirect($link, $msg);
	}
/**
 ...
 */
	...
	function removeReporting(){
		...
		if(count($cid)){
			for($i = 0; $i < count($cid); $i++){
				...
				$query = "DELETE FROM #__jblance_report_reporter WHERE report_id=".$db->quote($curr_bid);
				$db->setQuery($query);
				if(!$db->execute()){
					JError::raiseError($db->getErrorNum(), $db->getErrorMsg());
				}
	...
	function purgeReporting(){
		...
		for($i = 0; $i < count($reports); $i++){
			...
			//remove from report table
			$query = "DELETE FROM #__jblance_report WHERE id=".$db->quote($report->id);
			$db->setQuery($query);
			if(!$db->execute()){
				JError::raiseError($db->getErrorNum(), $db->getErrorMsg());
			}
			...
			//remove from reporter table
			$query = "DELETE FROM #__jblance_report_reporter WHERE report_id=".$db->quote($report->id);
			$db->setQuery($query);
			if(!$db->execute()){
				JError::raiseError($db->getErrorNum(), $db->getErrorMsg());
			}
