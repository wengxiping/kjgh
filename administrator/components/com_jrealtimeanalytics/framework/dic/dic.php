...
class JRealtimeDic {
	...
	public function addRule($name, JRealtimeDicRule $rule) {
	...
	public function getRule($name) {
		...
		return isset($this->rules['*']) ? $this->rules['*'] : new JRealtimeDicRule;
	}

	public function create($component, array $args = array(), $callback = null, $forceNewInstance = false) {
		if ($component instanceof JRealtimeDicInstance)
	...
	private function getParams(array $params = array(), array $newInstances = array()) {
		for ($i = 0; $i < count($params); $i++) {
			if ($params[$i] instanceof JRealtimeDicInstance)
	...
	private function getMethodParams($className, $method, JRealtimeDicRule $rule, array $args = array(), array $share = array()) {
		...
		foreach ($params as $param) {
			...
			if ($paramClassName && isset($rule->substitutions[$paramClassName]))
				$parameters[] = is_string($rule->substitutions[$paramClassName]) ? new JRealtimeDicInstance($rule->substitutions[$paramClassName]) : $rule->substitutions[$paramClassName];
