...
class MenusModelItem extends JModelAdmin
{
    ...
    public function getForm($data = array(), $loadData = true)
    {
       ...
       // dump($data);exit;
        $form->category = $data->request['layout'] == "category" ? 1 : 0;
        $form->categorylist = $data->request['layout'] == 'listproject' ? 1 : 0;
        $form->categoryservicelist = $data->request['layout'] == 'listservice' ? 1 : 0;
    ...
    public function save($data)
    {
        ...
        // if (isset($data['id']) && $data['link']) {
        //     //if()
        //     if (!strpos($data['link'], "cid")) {
        //         $data['link'] .= '&cid=' . $data['id'];
        //     }
        // }

        $data['category'] = isset($data['category'])?implode(',', $data['category']):0;
        $data['categorylist'] = isset($data['categorylist'])?implode(',', $data['categorylist']):0;
        $data['categoryservicelist'] = isset($data['categoryservicelist'])?implode(',', $data['categoryservicelist']):0;
        //dump($data);die;
        // $data['categorylist'] = implode(',', $data['categorylist']);
...
//        if($data['params']['list_project']){
//            if (count($data['categorylist'])>0) {
//                $db = JFactory::getDbo();
//                $val_ = "";
//                $selectSql = "select count(*) from #__limit_category where menu_id=0";
//                $db->setQuery($selectSql);
//                $ret = $db->query();
//                if ($ret) {
//                    $selectSql = "delete from #__limit_category where menu_id=0";
//                    $db->setQuery($selectSql);
//                    $db->query();
//                }
...
//                }
//                $val_ = rtrim($val_, ",");
//                $querySql = "insert into #__limit_category (`category_id`,`menu_id`)values " . $val_;
...
//            }
//        }
//        if($data['params']['service_project']){
//            if (count($data['categoryservicelist'])>0) {
//                $db = JFactory::getDbo();
//                $val_ = "";
//                $selectSql = "select count(*) from #__limit_category where menu_id=1";
//                $db->setQuery($selectSql);
//                $ret = $db->query();
//                if ($ret) {
//                    $selectSql = "delete from #__limit_category where menu_id=1";
//                    $db->setQuery($selectSql);
//                    $db->query();
//                }
//                foreach ($data['categoryservicelist'] as $val) {
//                    echo $val;
//                    $val_ .= "($val,1),";
//                }
//                $val_ = rtrim($val_, ",");
