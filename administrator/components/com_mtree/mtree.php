...
// Access check.
if (!JFactory::getUser()->authorise('core.manage', 'com_mtree'))
{
...
if($task != 'upgrade')
{
	...
	require_once( JPATH_ROOT . '/components/com_mtree/controllers/listings.php' );
...
function editlink( $link_id, $cat_id, $for_approval=false, $option ) {
	...
	$sql = "SELECT cf.*, " . ($row->link_id ? $row->link_id : 0) . " AS link_id, cfv.value, cfv.attachment, cfv.counter"
		...
		.	"\n("
		.	" cf.published='1'"
		// If no custom fields are assigned and published, make sure the link_name (cf_id: 1) is shown.
	...
	$fields = new mFields();
	$fields->setCoresValue( $row->link_name, $row->link_desc, $row->firstname, $row->lastname, $row->address, $row->city, $row->state, $row->country, $row->postcode, $row->contactperson, $row->mobile, $row->date, $row->year, $row->telephone, $row->fax, $row->email, $row->website, $row->price, $row->link_hits, $row->link_votes, $row->link_rating, $row->link_featured, $row->link_created, $row->link_modified, $row->link_visited, $row->publish_up, $row->publish_down, $row->metakey, $row->metadesc, $row->user_id, $row->owner );
	...
	$form->setValue('link_modified', 'publishing', $row->link_modified);
	...
	switch( $act ) {
		...
		case "approve":
			...
			if( $mtconf->get('reset_created_date_upon_approval') ) {
				$post['publishing']['link_created'] = $jdate->toSql(true);
			}
			savelink( $option, $post );
			break;
	}
...
function savelink( $option, $post=null ) {
	...
	$publishing['link_votes'] = applyCoreFieldParseValue('link_votes', $publishing['link_votes'], $row->link_id);
	$publishing['link_hits'] = applyCoreFieldParseValue('link_hits', $publishing['link_hits'], $row->link_id);
	$publishing['link_visited'] = applyCoreFieldParseValue('link_visited', $publishing['link_visited'], $row->link_id);

	$row->bind($publishing);

	// Checking listing alias has to be done after binding publishing data because $publishing may overwrite $row->alias
	// with empty alias, thus leaving the listing without an alias.
	# Listing alias
	if( empty($row->alias) )
	{
	...
	if ($row->link_id == 0) {
		...
		if( empty($row->link_created) ) {
			$row->link_created = JFactory::getDate()->toSql();
		}

		if( empty($row->publish_up) )
		{
			$row->publish_up = $row->link_created;

		} elseif ($row->publish_up && strlen(trim( $row->publish_up )) <= 10) {
	...
	// 2) Modification request from listing owner
        ...
        $link_created = JFactory::getDate($row->link_created, $tz);
        $row->link_created = $link_created->toSQL();

        $publish_up = JFactory::getDate($row->publish_up, $tz);
        $row->publish_up = $publish_up->toSQL();

		$row->link_modified = $row->getLinkModified( (empty($original_link_id)?$row->link_id:$original_link_id), $post );

		// Get other info from original listing. This will be used in on onAfterModifyListing plugin event.
		$database->setQuery( "SELECT * FROM #__mt_links WHERE link_id = '$row->link_id' LIMIT 1" );
		$original = $database->loadObject();

		if( $original->link_published != $row->link_published ) {
			JPluginHelper::importPlugin('mosetstree');
			$dispatcher->trigger('onContentChangeState', array('com_mtree.listing', $row->link_id, $row->link_published));
		}
		...
		$link_approved = $original->link_approved; // 1: approved; 0:unapproved/new listing; <-1: pending approval for update
	...
	else
	{
		...
		$row->publish_down = JFactory::getDate($row->publish_down, $tz)->toSql();
	}
	...
	$sql = 'SELECT cf_id, counter FROM #__mt_cfvalues WHERE link_id= ' . $database->quote($row->link_id) . ' AND counter > 0';
	$database->setQuery($sql);
	...
	// This will be used near the end of the execution to see if we should execute onAfterNewListingApproved
	$is_approval_of_a_new_listing = false;
	...
	if ( $row->link_approved == 1 && $row->link_id > 0 ) {
		...
		# This is an approval of a new listing (not approval of a modification)
		if ( $link_approved <= 0 ) {
			$row->updateLinkCount( 1 );
			$row->approveLink();

			$is_approval_of_a_new_listing = true;
		}
	}
	...
	if(count($fieldtype) > 0 ) {
		...
		{
			loadFieldTypeClass($ft->field_type);
		}
	}
	...
	# Perform parseValue on Core Fields
			...
			$v = applyCoreFieldParseValue( $k, $v, $row->link_id);
	...
	if( !empty($raw_filenames) )
	{
		...
		$removed_attachments = $raw_filenames;
	}

	# Keep track of all the uploaded and removed Images for use in Plugin events.
	$added_images = array();
		...
		if( !is_writable($mtconf->getjconf('absolute_path').$mtconf->get('relative_path_to_listing_small_image')) || !is_writable($mtconf->getjconf('absolute_path').$mtconf->get('relative_path_to_listing_medium_image')) ||  !is_writable($mtconf->getjconf('absolute_path').$mtconf->get('relative_path_to_listing_original_image')) ) {
			...
			for($i=0;$i<count($files['image']['name']);$i++) {
				if ( !empty($files['image']['name'][$i]) && $files['image']['error'][$i] == 0 &&  $files['image']['size'][$i] > 0 ) {
					...
					$added_images[] = $img_id . '.' . $file_extension;
				}
			}
		}
	}
	...
	$dispatcher->trigger('onContentAfterSave', array('com_mtree.listing', new JObject(array('link_id' => $row->link_id, 'link_name' => $row->link_name)), $new_link));

	if($new_link) {
		// This is a submission of a new listing. Fire onAfterCreateListing event.
		$dispatcher->trigger('onAfterCreateListing', array((array)$row, $active_cfs, $cat_id, $added_images) );
	} else {
		// This is a modification to an existing listing. Fire onAfterModifyListing event.
		$dispatcher->trigger('onAfterModifyListing', array((array)$original,$original_cfs,(array)$row,$active_cfs, $removed_attachments, $original->link_id, $cat_id, $added_images, $removed_images) );
	}

	if( $is_approval_of_a_new_listing ) {
		$dispatcher->trigger('onAfterNewListingApproved', array($row->link_id) );
	}
...
// /*
	if ( $returntask <> '' ) {
		...
		$task = JFilterInput::getInstance()->clean($post['task'], 'cmd');
...
function loadFieldTypeClass($field_type)
{
	global $mtconf;

	$class_name = 'mFieldType_' . $field_type;
	if( !class_exists($class_name) ) {
		$fieldtype_file = JPATH_ROOT . $mtconf->get('relative_path_to_fieldtypes') . $field_type . '/'  . $field_type . '.php';
...
function applyCoreFieldParseValue( $field, $value, $link_id)
{
	$database = JFactory::getDbo();
	static $core_params;

	if(strpos(strtolower($field),'link_') === false) {
		$core_field_type = 'core' . $field;
	} else {
		$core_field_type = 'core' . str_replace('link_','',$field);
	}
	$class = 'mFieldType_' . $core_field_type;

	if(!class_exists($class))
	{
		loadFieldTypeClass($core_field_type);
	}
	...
	if(class_exists($class)) {
		...
		$mFieldTypeObject = new $class(array('linkId'=>$link_id,'params'=>$core_params[$core_field_type]->params));
		$v = call_user_func(array(&$mFieldTypeObject, 'parseValue'),$value);
...
function publishLinks( $link_id=null, $publish=1,  $option ) {
	...
	if (!$database->execute()) {
		...
		JPluginHelper::importPlugin('finder');
		JPluginHelper::importPlugin('mosetstree');
		$dispatcher->trigger('onContentChangeState', array('com_mtree.listing', $link_id, (int)$publish));
	}
...
function listcats( $cat_id, $cat_parent, $option ) {
	...
	# Get the list of additional listings column
	$database->setQuery( "SELECT REPLACE(field_type, 'core', '') FROM #__mt_customfields WHERE shown_in_backend_listings_column = '1'" );
	$core_field_columns = $database->loadColumn();

	HTML_mtree::listcats( $cats, $links, $softlink_cat_ids, $core_field_columns, $mtCats, $catPageNav, $pageNav, $pathWay, $option );
}
...
function savecat( $option, $show ) {
	...
	if( !empty($post['override']) )
	{
		...
		foreach($post['override'] AS $override => $value)
		{
            ...
            if( substr($override, -7) == '_order1' )
            {
                $order2_key = substr($override, 0, -1) . '2';
                $params[$order2_key] = $post['config'][$order2_key];
            }
		}
...
function approve_links( $link_id, $publish=0, $option ) {
	...
	if (count( $link_id )) {
		...
		foreach( $link_id AS $lid ) {
			...
			if( $mtLinks->link_approved == 0 )
			{
				$dispatcher->trigger('onContentAfterSave', array('com_mtree.listing', new JObject(array('link_id' => $lid, 'link_name' => $mtLinks->link_name)), false));
				$dispatcher->trigger('onAfterNewListingApproved', array($lid) );
			}
...
function search( $option ) {
	...
	$limit = $app->getUserStateFromRequest( "viewlistlimit".__FUNCTION__.$search_where, 'limit', $mtconf->getjconf('list_limit') );
	$limitstart = $app->getUserStateFromRequest( "viewcli{$option}limitstart".__FUNCTION__.$search_where, 'limitstart', 0 );
	...
	if ( $search_where == 1 ) {
		...
		if ($id_found) {
			...
			$sql_order_by = $listings->getSQLOrderBy();
	...
	if ( $search_where == 1 )
	{
		...
		// Links
		HTML_mtree::searchresults_links( $results, $pageNav, $pathWay, $search_where, $search_text, $core_field_columns, $option );
	} else {
		// Categories
	...
	if ( $search_where == 1 ) {
		...
		switch ($searchParams['publishing']) {
			...
			case 3: // Unpublished
				...
				break;
			case 4: // Pending
				$advsearch->addRawCondition( "(publish_up >= '$now' AND link_published = '1' )" );
				break;
			case 5: // Expired
...
* My Subscription
*/
function mySubscription( $option )
{
	HTML_mtree::mySubscription($option);
}
...
function template_pages( $option ) {
	...
	$template_files = JFolder::files($mtconf->getjconf('absolute_path') . '/components/com_mtree/templates/' . $template, '(\.tpl\.php|\.css)$', false, false);
...
function edit_templatepage( $option ) {
	...
	$file = JPath::clean($mtconf->getjconf('absolute_path') .'/components/com_mtree/templates/'. $template .'/'. $page );
...
function copy_template2( $option ) {
	...
	{
		$app->redirect( "index.php?option=$option&task=templates", JText::_( 'COM_MTREE_TEMPLATE_PLEASE_SPECIFY_A_NAME' ), 'error' );
	}

	if( empty($new_template_folder) )
	{
		$app->redirect( "index.php?option=$option&task=templates", JText::_( 'COM_MTREE_TEMPLATE_PLEASE_SPECIFY_A_FOLDER_NAME' ), 'error' );
	}
...
function install_template( $option ) {
	...
	// Move uploaded file
	$moved = JFile::upload($tmp_src, $tmp_dest, false, true);
	...
	{
		$xmlDoc = simplexml_load_file($tmp_xml);
...
function import_images( $option, $cf_id, $limitstart, $limit )
{
	...
	$chunked_images = array_chunk($images,3); //chunk into groups of 3
	...
	foreach($chunked_images AS $images) {
		...
		foreach($images as $image) {
			$ch = curl_init(trim($image->value));
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
			curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/56.0.2924.87 Safari/537.36');
		...
		foreach($handles as $handle) {
			$image = $images[$j];
			$storedImageName = $images[$j]->link_id . '_' . basename($image->value);
			$path_to_file = JPATH_ROOT . $mtconf->get( 'relative_path_to_listing_original_image' ) . $storedImageName;
			file_put_contents($path_to_file, curl_multi_getcontent($handle));

			// Copy the image to medium and small size directory
			copy( $path_to_file, JPATH_ROOT . $mtconf->get( 'relative_path_to_listing_medium_image' ) . $storedImageName);
			copy( $path_to_file, JPATH_ROOT . $mtconf->get( 'relative_path_to_listing_small_image' ) . $storedImageName);
...
function saveconfig($option, $show) {
	...
	# We have a configcode called 'taggable_fields' that shows a list of checkboxes. This allows users to select zero or
	# more taggable fields. If none are selected, the 'Save configs' SQLs below will not clear the checkboxes. So we
	# need clear all values for 'taggable_fields' configs and let the routine below to populate them if there are
	# selected checkboxes.

	$sql = 'UPDATE #__mt_config SET value = \'\' WHERE configcode = \'taggable_fields\'';
	$database->setQuery($sql);
	$database->execute();
	...
	foreach( $post AS $key => $value ) {
		...
		// If site is in demo mode, only allowed config changes for displayed configuration.
		if( $mtconf->get('demo_mode') )
		{
			$sql .= ' AND displayed = 1';
		}
...
function updateMtUpdateUrlWithAccessKey($value)
{
	$db = JFactory::getDBO();

	$updateUrl = 'https://update.mosets.com/manifest?name=pkg_mtree&key=' . $value;

	$sql = 'SELECT extension_id FROM #__extensions WHERE element = ' . $db->quote('com_mtree') . ' LIMIT 1';
	$db->setQuery($sql);
	...
	$sql = 'SELECT update_site_id FROM #__update_sites_extensions WHERE extension_id = ' . $db->quote($extension_id) . ' LIMIT 1';
	$db->setQuery($sql);
	$update_site_id = $db->loadResult();

	if(empty($update_site_id))
	{
		/* #__update_sites_extensions is a table that maps extension_id with update_site_id.
		 * #__update_sites is the main table that stores extension name and the location URL for an update.
		 *
		 * If Mosets Tree is not found in this table (#__update_sites_extensions), we assume MT entry does not exists in
		 * #__update_sites as well. Therefore we are adding them here.
		 */

		$update_site_id = updateMtInsertUpdateSite($value);
		updateMtInsertUpdateSiteExtension($update_site_id, $extension_id);

	}
	else
	{
		$sql = 'UPDATE #__update_sites SET location = ' . $db->quote($updateUrl);
		$sql .= ' WHERE update_site_id = ' . $db->quote($update_site_id) . ' LIMIT 1;';
		$db->setQuery($sql);
		$db->execute();
	}
...
function updateMtInsertUpdateSiteExtension($update_site_id, $extension_id)
{
	$db = JFactory::getDBO();

	$updateSiteExtension = new stdClass();
	$updateSiteExtension->update_site_id = $update_site_id;
	$updateSiteExtension->extension_id = $extension_id;

	$db->insertObject("#__update_sites_extensions", $updateSiteExtension);
	return $db->execute();
}

function updateMtInsertUpdateSite($key)
{
	$db = JFactory::getDBO();

	$updateUrl = 'https://update.mosets.com/manifest?name=pkg_mtree&key=' . $key;

	$updateSite = new stdClass();
	$updateSite->name = 'Mosets Tree';
	$updateSite->type = 'extension';
	$updateSite->location = $updateUrl;
	$updateSite->enabled = 1;

	$db->insertObject("#__update_sites", $updateSite);
	return $db->insertid();
}

function isSubscriptionActive($dateExpiry)
{
...
function getMTUrl( $query )
{
	...
	// Supports sh404sef. If sh404sef is in used, use its router instead
	/// of Joomla's
	if ( defined('SH404SEF_IS_RUNNING') && SH404SEF_IS_RUNNING )
	{
		return Sh404sefHelperGeneral::getSefFromNonSef( $url, $fullyQualified = true, $xhtml = false, $ssl = null);
	}
	else
	{
...
function bootstrapRadioBoolean($name, $selected, $disabled=false, $captionTrue = null, $captionFalse = null) {
?>
<fieldset class="radio btn-group" id="<?php echo $name; ?>_fieldset">
	<?php
	if(is_null($captionTrue) || is_null($captionFalse) )
	{
		$captionTrue = JText::_( 'JYES' );
		$captionFalse = JText::_( 'JNO' );
	}
