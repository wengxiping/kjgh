...
class HTML_mtgeocode {
	...
	jQuery(document).ready(function(){
		...
		<?php 
		$address_parts = array('link_name','address','city','state','postcode','country');
		foreach( $listings AS $listing ) {
			echo "\n";
			echo 'addRow('.addslashes($listing->link_id);

			foreach( $address_parts AS $address_part )
			{
				echo ', ';

				$address_data = json_encode(addslashes($listing->{$address_part}));
