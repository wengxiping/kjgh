...
function editcf( $cf_id, $option ) {
	...
	if ($row->cf_id == 0) {
		...
		$row->hidden = 0;
		$row->published = 1;
		$row->view_access_level = 1;
		$row->edit_access_level = 1;
		$row->size = 30;
		$row->hide_caption = 0;
		...
		$row->summary_view=0;
		$row->tag_search=0;
		$row->shown_in_backend_listings_column=0;
	} else {
	...
	$cf_types = array (
		...
		// 'mselectmultiple' => JText::_( 'COM_MTREE_FIELD_TYPE_MSELECTMULTIPLE' ),
	...
	# Get access levels
	$database->setQuery( 'SELECT id, title FROM #__viewlevels ORDER BY ordering ASC' );
	$access_levels	= $database->loadObjectList();
	...
	$lists['view_access_level'] = JHtml::_('select.genericlist', $jhtml_access_levels, 'view_access_level', 'size="1"', 'value', 'text', intval($row->view_access_level));
	$lists['edit_access_level'] = JHtml::_('select.genericlist', $jhtml_access_levels, 'edit_access_level', 'size="1"', 'value', 'text', intval($row->edit_access_level));
...
function savecf( $option ) {
	...
	$post	= $_POST;
	$post['prefix_text_mod'] = JFactory::getApplication()->input->get('prefix_text_mod', '', 'raw');
	$post['suffix_text_mod'] = JFactory::getApplication()->input->get('suffix_text_mod', '', 'raw');
	$post['prefix_text_display'] = JFactory::getApplication()->input->get('prefix_text_display', '', 'raw');
	$post['suffix_text_display'] = JFactory::getApplication()->input->get('suffix_text_display', '', 'raw');
