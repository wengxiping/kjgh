...
function tellDateTime( $datetime ) {
	...
	$time_now = (new JDate('now'))->toUnix();
	$time_str = (new JDate( $datetime ))->toUnix();
...
class mtTree {
	...
	function rebuild($parent, $left) { 
		$database = JFactory::getDBO();
		 ...
		 $database->setQuery("SELECT cat_id FROM #__mt_cats WHERE cat_parent = " . intval($parent) . ' ORDER BY lft ASC');
