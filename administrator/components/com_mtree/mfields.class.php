...
class mFields {
	...
	function __construct( $fieldsObjectList=null ) {
		$this->pointer = 0;
	...
	function loadField( $fieldsObject ) {
		...
		$this->fields[] = array(
			...
			'viewAccessLevel' => $fieldsObject->view_access_level,
	...
	function setCoresValue( $link_name, $link_desc, $firstname, $lastname, $address, $city, $state, $country, $postcode, $contactperson, $mobile, $date, $year, $telephone, $fax, $email, $website, $price, $link_hits, $link_votes, $link_rating, $link_featured, $link_created, $link_modified, $link_visited, $publish_up, $publish_down, $metakey, $metadesc, $user_id, $username ) {
		$this->coresValue['link_name'] = $link_name;
		$this->coresValue['link_desc'] = $link_desc;
		$this->coresValue['firstname'] = $firstname;
		$this->coresValue['lastname'] = $lastname;
		...
		$this->coresValue['contactperson'] = $contactperson;
		$this->coresValue['mobile'] = $mobile;
		$this->coresValue['date'] = $date;
		$this->coresValue['year'] = $year;
		...
		while( $this->hasNext() ) {
			...
			foreach( $searchFields AS $searchField ) {
				if( isset($post[$searchField]) && $post[$searchField] != '') {
					...
					{
						if( !is_array($this->fields[$this->pointer]['searchValue']) ) {
							$this->fields[$this->pointer]['searchValue'] = array();
						}
					...
					elseif( 
						...
						(isset($post['cfcat']) && $field->getName() == $post['cfcat'])
					)
					{
	...
	function hasNext() {
		if( is_array($this->fields) && count($this->fields) > 0 && array_key_exists($this->pointer,$this->fields) ) {
			return true;
		} else {
			return false;
		}
...
class mFieldType {
	...
	function hasSearchField() { return ( $this->getNumOfSearchFields() <= 0 ) ? false:true; }
	...
	function hasJSInit() {
		if($this->getJSInitFunction() != '' && !is_null($this->getJSInitFunction())) {
			return true;
		} else {
			return false;
		}
	...
	function hasJSRemove() {
		if($this->getJSRemoveFunction() != '' && !is_null($this->getJSRemoveFunction())) {
			return true;
		} else {
			return false;
		}
	...
	/**
	 * Method to prepare field's search parameters before being evaluated for
	 * searching. This will be executed to all searchable fields even when
	 * there are no values given.
	 *
	 * In listlisting_cache method, it only checks for the first search
	 * parameter to evaluate whether the field is eligible for searching.
	 *
	 * For example, a field with 2 search input (cf42 & cf42_2), cf42 must
	 * contain value in order for it to be searched. There are instances where
	 * it is useful to be able to provide values to second parameter only. This
	 * method provide opportunity to check this and populate the first search
	 * input in order for it to be eligible for searching.
	 *
	 * @param	Array		$searchParams	Array of search parameters
	 *
	 * @return  Array       Returns the prepared search parameters. If no
	 *                      changes are made, the array are returned
	 *                      unmodified
	 *
	 * @since 3.6
	 */
	public function prepareSearchParams( $searchParams )
	{
		return $searchParams;
	}
	...
	function getSearchValue() {
		...
		// Check if cookie contains search value information
			...
			elseif( $numOfSearchFields > 1 )
			{
				...
				for($i = 1; $i <= $this->getNumOfSearchFields(); $i++)
				{
	...
	function getSearchHTML( $showSearchValue=false, $showPlaceholder=false, $idprefix='search_' ) {
		...
		if( !empty($this->arrayFieldElements[0]) )
		{
			...
			if( $showSearchValue && $this->getSearchValue() !== false ) {
				...
				{
					$html .= htmlspecialchars(array_shift($searchValue));
				} else {
					$html .= htmlspecialchars($searchValue);
				}
			}
	...
	function getHiddenHTML() {
		...
		if( $this->hasSearchValue() )
		{
			if( is_array($searchValue) ) {
				...
				$html .= ' type="hidden"';
				$html .= ' name="'.$this->getName().'"';
				$html .= ' value="'.htmlspecialchars($this->getSearchValue()).'"';
				$html .= ' />';
			}
			return $html;
		} else {
	...
	function getJSRemove() {
		if( $this->hasJSRemove() ) {
			$js = $this->getJSRemoveFunction();
			return $js;
		} else {
			return null;
		}
	...
	*/
	...
	function getSearchFieldName($count=1) {
		if($count == 1) {
			return $this->getName();
		} elseif( $count <= $this->getNumOfSearchFields() ) {
	...
	*/
	function getOutput($view=1) {
		if( $this->tagSearch && $this->hasValue() )
		{
			...
			for($i=0;$i<$countTags;$i++)
			{
				...
				$outputTags[$i] = '';
				$outputTags[$i] .= $this->getTagLinkHtml( $arrTags[$i] );
			}
			$html = '';
	...
	function getWhereCondition() {
		if( func_num_args() == 0 ) {
			...
			if( $this->isCore() ) {
				...
				$searchValue = func_get_arg(0);
				if( is_array($searchValue) ) {
					$searchValue = array_shift( $searchValue );
				}

				return '(cfv#.value LIKE \'%' . JFactory::getDBO()->escape( $searchValue, true ) . '%\')';
			}
		}
	}
	...
	function linkcreator( $matches ) {	
		$url = $matches[1];
		$append = '';
		if ( in_array(substr($matches[2],-1), array('.',')')) ) {
			$url .= substr($matches[2], 0, -1);
			$append = substr($matches[2],-1);

		# Prevent cutting off breaks <br />
		} elseif( substr($matches[2],-3) == '<br' ) {
			$url .= substr($matches[2], 0, -3);
			$append = substr($matches[2],-3);

		} elseif( substr($matches[2],-1) == '>' ) {
			$regex = '/<(.*?)>/i';
			preg_match( $regex, $matches[2], $tags );
			if( !empty($tags[1]) ) {
				$append = '<'.$tags[1].'>';
				$url .= $matches[2];
		...
		return '<a href="'.$url.'" rel="nofollow" target="_blank">'.$url.'</a>'.$append.' ';
	}
	...
	/**
	 * Method to generate a unique HTML class name to be used for field's values.
	 *
	 * @param $value String Field's value
	 *
	 * @return string Class name
	 */
	function getValueHtmlClassName( $value )
	{
		$className = 'mt-field-' . $this->id . '-value-';
		$className .= JFilterOutput::stringURLSafe($value);

		return $className;
	}

	/**
	 * Method to generate HTML link for tagged value.
	 *
	 * @param $value String Field's value
	 *
	 * @return string
	 */
	function getTagLinkHtml( $value )
	{
		$encodedValue = str_replace('&', '%26', $value);
		$encodedValue = urlencode($encodedValue);

		$html = '<a';
		$html .= ' rel="tag"';
		$html .= ' class="tag ';
		$html .= $this->getValueHtmlClassName( $value );
		$html .= '"';
		$html .= ' href="'.( JRoute::_('index.php?option=com_mtree&task=searchby&cf_id='.$this->getId().'&value='.$encodedValue) ).'"';
		$html .= '>';
		$html .= $value;
		$html .= '</a>';
...
class mFieldType_telephone extends mFieldType
{
	function getInputHTML()
	{
		$html = '';
...
class mFieldType_weblink extends mFieldType {
	...
	function getInputHTML()
	{
		...
		if($showGo && $this->inBackEnd())
		{
			...
			$html .= 'javascript:window.open("index.php?option=com_mtree&task=openurl&url="+encodeURI(document.getElementById("' . $this->getInputFieldID(1) . '").value))\'>';
	...
	function getJSValidationFunction() {
        return 'function(){return /^(?:(?:http|https):\/\/)?(?:\S+(?::\S*)?@)?(?:(?!(?:10|127)(?:\.\d{1,3}){3})(?!(?:169\.254|192\.168)(?:\.\d{1,3}){2})(?!172\.(?:1[6-9]|2\d|3[0-1])(?:\.\d{1,3}){2})(?:[1-9]\d?|1\d\d|2[01]\d|22[0-3])(?:\.(?:1?\d{1,2}|2[0-4]\d|25[0-5])){2}(?:\.(?:[1-9]\d?|1\d\d|2[0-4]\d|25[0-4]))|(?:(?:[a-z\u00a1-\uffff0-9]-*)*[a-z\u00a1-\uffff0-9]+)(?:\.(?:[a-z\u00a1-\uffff0-9]-*)*[a-z\u00a1-\uffff0-9]+)*(?:\.(?:[a-z\u00a1-\uffff]{2,})))(?::\d{2,5})?(?:\/\S*)?$/i.test(arguments[0].value)}';
    }
...
class mFieldType_selectmultiple extends mFieldType
{
	...
	function getSearchHTML( $showSearchValue=false, $showPlaceholder=false, $idprefix='search_' ) {
		$params['searchFieldType'] = intval($this->getParam('searchFieldType','1'));
...
class mFieldTypeNumber extends mFieldType {
	...
	public function getSearchFieldType()
	{
		// 1: Exact
		// 2: Contains
		// 3: From X To Y (Default)
		// 4: Exactly/More Than/Less Than X
		return $this->getParam('searchFieldType','3');
	}

	public function prepareSearchParams( $searchParams )
	{
		// From X To Y
		if ($this->getSearchFieldType() == 3)
		{
			// When the From value is empty, replace it with 0.
			if ($searchParams[0] === '' && !empty($searchParams[1]))
			{
				return array(0, $searchParams[1]);
			}
		}
	...
	public function getNumOfSearchFields()
	{
		if ( in_array($this->getSearchFieldType(),array(1,2)) )
		{
			return 1;
		}
	...
	function getInputHTML()
	{
		$html = '<input'
				...
				. $this->getDataValidatorAttr()
				. ' class="'.($this->isRequired() ? ' required':'')
				. '" type="number" name="' . $this->getInputFieldName(1)
				. '" id="' . $this->getInputFieldID(1)
				. '" size="' . ($this->getSize()?$this->getSize():'30');
		$html .= '" value="' . htmlspecialchars($this->getInputValue()) ;
		$html .= '" />';
	...
	function getHiddenHTML() {
		...
		switch($this->getSearchFieldType())
		{
			case 1:
			case 2:
			case 4:
				return parent::getHiddenHTML();
	...
	function getSearchHTML( $showSearchValue=false, $showPlaceholder=false, $idprefix='search_' ) {
		...
		switch($this->getSearchFieldType())
		{
			...
			case 1:
				// Contains
			case 2:
				$html .= parent::getSearchHTML(func_get_args());
				break;

			// From X To Y
			case 3:
			default:
				return self::getSearchHTMLForFromXToY(func_get_args());
				break;

			// Exactly/More Than/Less Than X
			case 4:
				return self::getSearchHTMLForExactlyMoreThanLessThanX(func_get_args());
				break;
		}
	...
	public function getSearchHTMLForFromXToY( $showSearchValue=false, $showPlaceholder=false, $idprefix='search_' )
	{
		$searchValue = $this->getSearchValue();

		$id = $this->getSearchFieldName(1);
		$uniqclass = uniqid('slider-');
		$selector_from = '#' . $this->getSearchFieldName(1) . '.' . $uniqclass;
		$selector_to = '#' . $this->getSearchFieldName(2) . '.' . $uniqclass;
		$selector_slider = '#slider-' . $id . '.' . $uniqclass;
		$selector_slider_display = '#slider-' . $id . '-display.' . $uniqclass;

		$currency_prefix = $this->getDisplayPrefixText();
		$currency_suffix = $this->getDisplaySuffixText();

		$searchSliderMin = intval($this->getParam('searchSliderMin', 0));
		$searchSliderMax = intval($this->getParam('searchSliderMax', 1000));
		$searchSliderStep = intval($this->getParam('searchSliderStep', 10));

		$search_value_from = $searchSliderMin;
		$search_value_to = $searchSliderMax;

		if( $showSearchValue ) {
			if( isset($searchValue[$this->getSearchFieldName(1)]) ) {
				$search_value_from = intval($searchValue[$this->getSearchFieldName(1)]);
			}

			if( isset($searchValue[$this->getSearchFieldName(2)]) ) {
				$search_value_to = intval($searchValue[$this->getSearchFieldName(2)]);
			}
		}
		...
		$html .= ' id="' . $this->getSearchFieldName(1) . '"';
		$html .= ' class="' . $uniqclass . '"';
		$html .= ' type="hidden"';
		if( $showSearchValue && isset($searchValue[$this->getSearchFieldName(1)]) ) {
			$html .= ' value="'.htmlspecialchars($searchValue[$this->getSearchFieldName(1)]).'"';
		}
		$html .='/>';

		$html .= '<input name="' . $this->getSearchFieldName(2) . '"';
		$html .= ' id="' . $this->getSearchFieldName(2) . '"';
		$html .= ' class="' . $uniqclass . '"';
		$html .= ' type="hidden"';
		if( $showSearchValue && isset($searchValue[$this->getSearchFieldName(2)]) ) {
			$html .= ' value="'.htmlspecialchars($searchValue[$this->getSearchFieldName(2)]).'"';
		}
		$html .='/>';

		$html .= '<div id="slider-' . $id . '" class="' . $uniqclass . '"></div>';
		$html .= '<div id="slider-' . $id . '-display" class="' . $uniqclass . '"></div>';

		$html .= <<<SLIDER
<script>
  jQuery(function() {
    jQuery( "$selector_slider" ).slider({
      range: true,
      min: $searchSliderMin,
      max: $searchSliderMax,
      step: $searchSliderStep,
      values: [ $search_value_from, $search_value_to ],
      slide: function( event, ui ) {
        jQuery( "$selector_from" ).val( ui.values[ 0 ] );
        jQuery( "$selector_to" ).val( ui.values[ 1 ] );

        jQuery( "$selector_slider_display" ).html( '$currency_prefix' + ui.values[ 0 ] + '$currency_suffix' + ' - ' + '$currency_prefix' + ui.values[ 1 ] + '$currency_suffix');

         if( ui.values[ 0 ] == 0 ) {
            jQuery( "$selector_from" ).val( '' );
         }
         if( ui.values[ 1 ] == $searchSliderMax ) {
	        jQuery( "$selector_slider_display" ).append( '+' );
            jQuery( "$selector_to" ).val( '' );
         }
      }
    });
        jQuery( "$selector_from" ).val( jQuery( "#slider-$id" ).slider( "values", 0 ) );
        jQuery( "$selector_to" ).val( jQuery( "#slider-$id" ).slider( "values", 1 ) );
        jQuery( "$selector_slider_display" ).html( '$currency_prefix' + jQuery( "$selector_slider" ).slider( "values", 0 ) + '$currency_suffix' + ' - ' + '$currency_prefix' + jQuery( "$selector_slider" ).slider( "values", 1 ) + '$currency_suffix');

         if( jQuery( "$selector_slider" ).slider( "values", 0 ) == 0 ) {
            jQuery( "$selector_from" ).val( '' );
         }

        if( jQuery( "$selector_slider" ).slider( "values", 1 ) == $searchSliderMax ) {
	        jQuery( "$selector_slider_display" ).append( '+' );
	        jQuery( "$selector_to" ).val('');
         }
...
SLIDER;
	...
	public function getSearchHTMLForExactlyMoreThanLessThanX( $showSearchValue=false, $showPlaceholder=false, $idprefix='search_' )
	{
		$operators = array(
			''	=> '',
	...
	function getWhereCondition() {
		...
		switch($this->getSearchFieldType())
		{
			// Exact
			case 1:
				return self::getWhereConditionForExact($args[0]);
				break;

			// Contains
			case 2:
				return parent::getWhereCondition($args[0]);
				break;

			// From X To Y
			case 3:
			default:
				return self::getWhereConditionForFromXToY($args[0], $args[1]);
				break;

			// Exactly/More Than/Less Than X
			case 4:
				return self::getWhereConditionForExactlyMoreThanLessThanX($args[0], $args[1]);
				break;
		}
	}

	public function getWhereConditionForExact() {
		$searchValue = func_get_arg(0);
		...
		if( $this->isCore() ) {
			return $this->getName() . ' = ' . (float) JFactory::getDBO()->escape( $searchValue, true ) . ' ';
		}

		return '(cfv#.value = ' . (float) JFactory::getDBO()->escape( $searchValue, true ) . ')';

	}

	public function getWhereConditionForFromXToY() {
		$args = func_get_args();

		// Only From value is provided.
		// From is provided, is numeric and non-zero.
		// To is not set, is zero or is empty.
		if( isset($args[0]) && is_numeric($args[0]) && $args[0] != 0 && (!isset($args[1]) || $args[1] == 0 || $args[1] == '') )
		{
			if( $this->isCore() )
			{
				return $this->getName() . ' >= ' . (float) $args[0] . ' ';
			}
			return '(cfv#.value >= ' . (float) $args[0] . ')';
		}

		// Only To value is provided
		// This will not happen because when only 'To' value is provided, the From will be given a zero value, thus the
		// if block below will be run

		// Both From & To values are provided.
		if( isset($args[0]) && isset($args[1]) && is_numeric($args[0]) && is_numeric($args[1]) )
		{
			if( $this->isCore() ) {
			...
			$where = '';
			$where .= '(';
			$where .= $fieldname . ' >= ' . (float) JFactory::getDBO()->escape( $args[0], true ) . ' ';
			$where .= ' AND ';
			$where .= $fieldname . ' <= ' . (float) JFactory::getDBO()->escape( $args[1], true ) . ' ';
			$where .= ')';
	...
	function getWhereConditionForExactlyMoreThanLessThanX() {
		$args = func_get_args();
...
class mFieldType_number extends mFieldTypeNumber {

	function getJSValidationFunction() {
		$thousandSeparator 	= $this->getParam('thousandSeparator',',');
		$decimalPoint 	= $this->getParam('decimalPoint','.');
		...
		if( in_array($this->getName(),array('link_hits','link_votes','link_visited','link_rating')) ) {
			...
			return 'function(){return(arguments[0].value != "" && /^-?(?:\d+|\d{1,3}(?:'.$thousandSeparator.'\d{3})+)?(?:\\'.$decimalPoint.'\d+)?$/i.test(arguments[0].value)==true);}';
		}
	}
	...
	function validateValue($value)
	{
		$thousandSeparator 	= $this->getParam('thousandSeparator',',');
		$decimalPoint 	= $this->getParam('decimalPoint','.');
		$numericValue = str_replace($decimalPoint, '.', str_replace($thousandSeparator, '', $value));
	...
	function getOutput($view=1) {
		$useThousandSeparator = $this->getParam('useThousandSeparator','1');
		$thousandSeparator = '';
		...
		$decimalPoint 	= $this->getParam('decimalPoint','.');
		$decimals 	= $this->getParam('decimals','0');

		$output = number_format((float)$this->getValue(), $decimals, $decimalPoint, $thousandSeparator);

		if( $this->tagSearch ) {
			return $this->getTagLinkHtml( $output );
		}
...
class mFieldType_date extends mFieldType {
	...
	function hasValue() {
		if( $this->value == '0000-00-00' ) return false;
	...
	function getInputHTML() {
		...
		$arrAttr = array('onchange'=>'handleValueChange(this);');
		if( $this->isRequired() ) {
			$arrAttr['required'] = true;
		}
	...
	function getJSInitFunction() {
		$fn = 'Calendar.setup({';
		$fn .= 'inputField: "' . $this->getInputFieldID() . '",';
		$fn .= 'ifFormat: "%Y-%m-%d",';
		$fn .= 'button: "' . $this->getInputFieldID() . '_img",';
		$fn .= 'align: "Tl",';
		$fn .= 'singleClick: true,';
		$fn .= 'firstDay: 0';
		$fn .= '})';
	...
	function getWhereCondition() {
		$args = func_get_args();
		if( isset($args[1]) && !empty($args[1]) ) $date0 = $args[1];
		if( isset($args[2]) && !empty($args[2]) ) $date1 = $args[2];
		if( isset($args[3]) && !empty($args[3]) ) $date2 = $args[3];
...
class mFieldType_year extends mFieldType {
	...
	function getSearchHTML( $showSearchValue=false, $showPlaceholder=false, $idprefix='search_' ) {
		...
		$html .= $this->getSearchOptionsHtml($showSearchValue);
		$html .= '</select>';
...
class mFieldType_email extends mFieldType {
	...
	function getJSValidationFunction() {
		return 'function(){return(/^[a-zA-Z0-9._\-\+]+@([a-zA-Z0-9.-]+\.)+[a-zA-Z0-9.-]{2,}$/i.test(arguments[0].value)==true)}';
	}
	...
	function validateValue( $value )
	{
		if (!preg_match("/^[a-zA-Z0-9._\-\+]+@([a-zA-Z0-9.-]+\.)+[a-zA-Z0-9.-]{2,}$/i", $value))
		{
...
class mFieldType_tags extends mFieldType {
	...
	private function _checkboxSearchHTML( $showSearchValue=false, $showPlaceholder=false, $idprefix='search_' ) {
		$i = 0;
		$html = '';
	...
	private function _dropdownSearchHTML( $showSearchValue=false, $showPlaceholder=false, $idprefix='search_' ) {
		$searchValue = $this->getSearchValue();
		...
		foreach( $rawTags AS $tag => $items )
		{
			$html .= '<option value="'.htmlspecialchars($tag).'"';
			if( $showSearchValue && $tag == $searchValue ) {
				$html .= ' selected=selected';
			}
			$html .= '>' . $tag . '</option>';
		}
	...
	function getSearchHTML( $showSearchValue=false, $showPlaceholder=false, $idprefix='search_' ) {
		$params['searchFieldType'] = $this->getParam('searchFieldType','text');

		switch($params['searchFieldType'])
		{
			case 'checkboxes':
				return self::_checkboxSearchHTML( $showSearchValue, $showPlaceholder, $idprefix );
				break;

			case 'dropdown':
				return self::_dropdownSearchHTML( $showSearchValue, $showPlaceholder, $idprefix );
				break;

			case 'text':
			default:
				return parent::getSearchHTML( $showSearchValue, $showPlaceholder, $idprefix );
				break;
	...
	private function _getCheckboxWhereCondition() {
		$params['searchOperator'] = intval($this->getParam('searchOperator','2'));
	...
	function getWhereCondition() {
		...
		{
			case 'checkboxes':
				return call_user_func_array(array('self', '_getCheckboxWhereCondition'),func_get_args() );
				break;

			case 'text':
			default:
				return call_user_func_array(array('parent', 'getWhereCondition'),func_get_args() );
				break;
		}
	...
	function parseValue($value) {
		$params['maxChars'] = intval($this->getParam('maxChars',80));
		$value = \Joomla\String\StringHelper::substr($value,0,$params['maxChars']);
		
		// Allow alphanumeric with dashes, and spaces
		$pattern = "/^[ŞşİıÇçĞğÜüÖöАаӐӑӒӓӘәӚӛӔӕБбВвГгҐґЃѓҒғӶӷҔҕДдЂђЕеЀѐЁёӖӗҼҽҾҿЄєЖжӁӂҖҗӜӝЗзҘҙӞӟӠӡЅѕИиЍѝӤӥӢӣІіЇїӀӀЙйҊҋЈјКкҚқҞҟҠҡӃӄҜҝЛлӅӆЉљМмӍӎНнҤҥҢңӉӊӇӈЊњОоӦӧӨөӪӫҨҩПпҦҧРрҎҏСсҪҫТтҬҭЋћЌќУуЎўӲӳӰӱӮӯҮүҰұФфХхҲҳҺһЦцҴҵЧчӴӵҶҷӋӌҸҹЏџШшЩщЪъЫыӸӹЬьҌҍЭэӬӭЮюЯяͰͱͲͳʹ͵Ͷͷͺͻͼͽ;΄΅Ά·ΈΉΊΌΎΏΐΑΒΓΔΕΖΗΘΙΚΛΜΝΞΟΠΡΣΤΥΦΧΨΩΪΫάέήίΰαβγδεζηθικλμνξοπρςστυφχψωϊϋόύώϏϐϑϒϓϔϕϖϗϘϙϚϛϜϝϞϟϠϡϢϣϤϥϦϧϨϩϪϫϬϭϮϯϰϱϲϳϴϵ϶ϷϸϹϺϻϼϽϾϿA-Za-z0-9- ]+$/";
