...
class mFieldType_mtext extends mFieldType {
	function getOutput($view=1) {
		...
		if($view == 1 AND $params['maxDetailsChars'] > 0 AND \Joomla\String\StringHelper::strlen($value) > $params['maxDetailsChars']) {
			$output .= $this->insertBreaks(\Joomla\String\StringHelper::substr($value,0,$params['maxDetailsChars']),$view);
			$output .= '...';
		} elseif($view == 2 AND $params['maxSummaryChars'] > 0 AND \Joomla\String\StringHelper::strlen($value) > $params['maxSummaryChars']) {
			$output .= $this->insertBreaks(\Joomla\String\StringHelper::substr($value,0,$params['maxSummaryChars']),$view);
			$output .= '...';
		} else {
	...
	function getInputHTML() {
		$params['maxAllowedChars'] 	= intval($this->getParam('maxAllowedChars',0));
		...
		switch($params['inputType'])
		{
			case 2:
				...
				if( $params['maxAllowedChars'] > 0 )
				{
					$html .= ' maxlength="'.$params['maxAllowedChars'].'"';
				}
	...
	function parseValue($value)
	{
		...
		if( $params['maxAllowedChars'] > 0 ) {
			$value  = \Joomla\String\StringHelper::substr($value, 0, $params['maxAllowedChars']);
		}
