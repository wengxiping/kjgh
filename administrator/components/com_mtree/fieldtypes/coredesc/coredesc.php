...
class mFieldType_coredesc extends mFieldType {
	...
	function parseValue($value) {
		...
		if($params['maxChars'] > 0) {
			$value = StringHelper::substr( $value, 0, $params['maxChars']);
		}
		return $value;
	}
	...
	function getOutput($view=1) {
		...
		// Summary view
			...
			if($params['summaryChars'] > 0) {
				$trimmed_desc = trim(StringHelper::substr($html,0,$params['summaryChars']));
			} else {
				$trimmed_desc = '';
			}
			...
			if (StringHelper::strlen($html) > $params['summaryChars']) {
				$output .= $trimmed_desc;
				$output .= JText::_( 'FLD_COREDESC_ELLIPSIS' );
			} else {
				$output = $html;
			}
			if( $params['showReadMore'] && ($params['whenReadMore'] == 1 || ($params['whenReadMore'] == 0 && StringHelper::strlen($html) > $params['summaryChars'])) ) {
				if(!empty($trimmed_desc)) {
					$output .= '<br />';
				}
				$output .= ' <a href="' . JRoute::_('index.php?option=com_mtree&task=viewlink&link_id=' . $this->getLinkId() . '&Itemid=' . $Itemid) . '" class="readon">' . $params['txtReadMore'] . '</a>';
			}
		}
		return $output;
	}
	...
	function getJSOnSave()
	{
		if( $this->isUsingWysiwygEditor() )
		{
			return JEditor::getInstance(JFactory::getConfig()->get('editor'))->save($this->getInputFieldId(1));
		}
		return null;
	}
