...
class mFieldType_monthYear extends mFieldType {
	...
	function getOutput($view=1) {
		$dateFormat = $this->getParam('dateFormat','%B %Y');
		$value = $this->getValue();
	...
	function getInputHTML() {
		...
		$html = '';
		$html .= '<select name="' . $this->getInputFieldName(1) . '" id="' . $this->getInputFieldId(1) . '">';
		...
		$html .= '<select name="' . $this->getInputFieldName(2) . '" id="' . $this->getInputFieldId(2) . '">';
		$html .= '<option value="">&nbsp;</option>';
