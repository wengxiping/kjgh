...
class mtPathWay {
	...
	*/
	...
	function printPathWayFromCat( $cat_id=0, $withlink=0, $skiproot=false ) {
		...
		{
			if ( $withlink ) {
				$return .= '<a href="'. JRoute::_('index.php?option=com_mtree&task=listcats&cat_id=0') .'">'.JText::_( 'COM_MTREE_ROOT' ).'</a> '.JText::_( 'COM_MTREE_ARROW' );
			} else {
	...
	function getBreadcrumbs( $cat_id )
	{
		$return = '';
		$return .= $this->printPathWayFromCat( $cat_id, 1, true );

		if ( $cat_id <> 0 ) {
			if ( 1 ) {
				$return .= '<a href="index.php?option=com_mtree&task=listcats&cat_id='.$cat_id.'">'.$this->getCatName( $cat_id ).'</a> ';
			} else {
				$return .= JText::_( 'COM_MTREE_ARROW' );
				$return .= $this->getCatName( $cat_id );
			}
	...
	static function getPathWay( $node=0 ) {
		$database = JFactory::getDBO();
		...
		if ( $node > 0 ) {
			...
			if ( !isset($cache_paths) || empty($cache_paths) || empty($cache_paths[$node]) ) {
				if(isset($cache_paths) && array_key_exists($node,$cache_paths)) {
					...
					$database->setQuery("SELECT cat_name, cat_id, lft, rgt FROM #__mt_cats WHERE cat_id = ".$database->quote((($node <= 0) ? self::cat_id : $node)));
...
class mtCats extends JTable {
	...
	*/
	function copyCategory( $cat_id, $dest, $copy_subcats, $copy_relcats, $copy_listings, $copy_reviews, $reset_hits, $reset_rating, $increment=null ) {
		...
		# Update alias by appending its ID so that it's unique.
		$this->_db->setQuery( 'UPDATE #__mt_cats SET alias = CONCAT(alias, '.$this->_db->quote($mtconf->get('sef_space')).', '.$this->_db->quote($new_cat_parent).') WHERE cat_id = ' . $this->_db->quote($new_cat_parent) );
		$this->_db->execute();
...
class mtLinks extends JTable {
	...
	*/
	function delLink( $link_id=0 ) {
		...
		# Remove all listing association
		$this->_db->setQuery("DELETE FROM #__mt_links_associations WHERE link_id2 = '".$link_id."'");
		if (!$this->_db->execute()) {
			echo "<script> alert('".$this->_db->getErrorMsg()."'); window.history.go(-1); </script>\n";
			return false;
		}
		...
		$object_listing = new JObject(array('link_id' => $link_id, 'link_name' => $this->link_name));
	...
	*/
	function approveLink( $link_id=0 ) {
		...
		if ( $link_approved < 0 ) {
			...
			if ( $mtconf->get('notifyuser_approved') == 1 ) {
				...
				if ( $email <> '' ) {
					$listing_url = getMTUrl( 'index.php?option=com_mtree&task=viewlink&link_id=' . $this->link_id );
					$subject	= JText::_( 'COM_MTREE_UPDATE_LISTING_APPROVED_SUBJECT' );
					$body 		= JText::sprintf( 'COM_MTREE_UPDATE_LISTING_APPROVED_MSG', $this->link_name, $listing_url );
		...
		// Approval to new listing
			...
			if ( $mtconf->get('notifyuser_approved') == 1 ) {
				...
				if( JAccess::checkGroup( 1 , 'mtree.listing.create', 'com_mtree' ) && !empty($this->email) ) {
					$email = $this->email;
				} else {
					$this->_db->setQuery( 
						"SELECT u.email FROM #__mt_links AS l"
				...
				if ( $email <> '' ) {
					...
					$body = JText::sprintf( 'COM_MTREE_NEW_LISTING_APPROVED_MSG', $this->link_name, $listing_url );
					JFactory::getMailer()->sendMail( $MailFrom, $FromName, $email, $subject, $body );
	 ...
	 * Renew Listing
	 *
	 * If a Save is done within the number of days configured in 'days_remaining_to_reactivate', update the
	 * publish_down date with another X days as configured in 'days_to_expire'.
	 *
	 * Return DateTime of the new publish_down date if renewed. False if not renewed.
	 */
	function renewLink($link_id=0) {
		...
		// Get listing publish_down data
		$this->_db->setQuery( "SELECT publish_down FROM #__mt_links WHERE link_id = " . $this->_db->quote($lid) . " LIMIT 1" );
		$publish_down = $this->_db->loadResult();

		// Renewal of listing expiration
		// If a Save is done within the number of days configured in 'days_remaining_to_reactivate', update the
		// publish_down date with another X days as configured in 'days_to_expire'.
		if(
				$mtconf->get('allow_listing_reactivation')
				&&
				$mtconf->get('days_to_expire') > 0
				&&
				$publish_down != $this->_db->getNullDate()
		) {
			$date_publish_down = new DateTime($publish_down);
			$date_today = new DateTime("now");
			$date_diff = $date_publish_down->diff($date_today);
			$days_remaining = $date_diff->format('%a');

			$new_publish_down = $date_publish_down;
			$new_publish_down->add(new DateInterval('P' . $mtconf->get('days_to_expire') . 'D'));

			if( $days_remaining <= $mtconf->get('days_remaining_to_reactivate') ) {
				// Renew listing
				$sql = "UPDATE #__mt_links "
					. " SET publish_down = " . $this->_db->quote($new_publish_down->format("Y-m-d H:i:s")) . " "
					. " WHERE link_id = " . $this->_db->quote($lid) . " LIMIT 1";
				$this->_db->setQuery( $sql );
				$this->_db->execute();

				return $new_publish_down;
			}
		}
	...
	function copyLink( $listing, $new_cat_parent, $reset_hits, $reset_rating, $copy_reviews, $copy_secondary_cats) {
		...
		$this->_db->setQuery( 'INSERT INTO #__mt_links 
			...
			. 'VALUES( '
				...
				. $this->_db->quote($org_listing->firstname) .  ', '
				. $this->_db->quote($org_listing->lastname) .  ', '
				...
				. $this->_db->quote($org_listing->contactperson) . ', '
				. $this->_db->quote($org_listing->mobile) . ', '
				. $this->_db->quote($org_listing->telephone) . ', '
				. $this->_db->quote($org_listing->date) . ', '
				. $this->_db->quote($org_listing->year) . ', '
		...
		# Copy listing's associations
		$this->_db->setQuery( 'INSERT INTO #__mt_links_associations (`link_id1`,`link_id2`) '
			. 'SELECT link_id1, \'' . $new_listing_id . '\' FROM #__mt_links_associations WHERE link_id2 = ' . $listing);
		$this->_db->execute();
...
class mtCustomFields extends JTable {
	...
	var $ordering=null;
	var $hidden=null;
	var $shown_in_backend_listings_column=null;
		...
		if (!$this->_db->execute()) {
			...
			if( $approve == 1 )
			{
				...
				if ( $mtconf->get('notifyowner_review_added') == 1 ) {
					...
					$this->_db->setQuery(
						'SELECT u.email, u.name, l.link_name, l.link_id FROM #__mt_reviews AS r'
						. ' LEFT JOIN #__mt_links AS l ON l.link_id = r.link_id'
						. ' LEFT JOIN #__users AS u ON u.id = l.user_id'
						. ' WHERE r.rev_id = ' . $rid . ' LIMIT 1'
						);
					...
					// Reviewer data
					$this->_db->setQuery(
							"SELECT u.name FROM #__mt_reviews AS r"
							.	"\nLEFT JOIN #__users AS u ON u.id = r.user_id"
							.	"\nWHERE r.rev_id = '".$rid."' LIMIT 1"
					);
					$reviewer = $this->_db->loadObject();

					$link_url = getMTUrl( 'index.php?option=com_mtree&task=viewlink&link_id=' . $row->link_id );

					if ( $row->email <> '' ) {
						$subject = JText::sprintf( 'COM_MTREE_REVIEW_ADDED_SUBJECT', $row->link_name, $reviewer->name );
 						$body = JText::sprintf( 'COM_MTREE_REVIEW_ADDED_MSG', $row->link_name, $link_url, $row->name, $reviewer->name );
...
class mtImage {
	...
	function __construct( $listing_image=null, $dest_dir=null ) {
