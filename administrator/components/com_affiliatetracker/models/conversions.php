...
class ConversionsModelConversions extends JModelLegacy
{
	...
	function __construct(){
		...
		$keywords = trim($mainframe->getUserStateFromRequest('affiliate.conversions.keywords','keywords','','string'));
		...
		$cal_start = $mainframe->getUserStateFromRequest('affiliate.conversions.cal_start','cal_start','','cmd');
		$cal_end = $mainframe->getUserStateFromRequest('affiliate.conversions.cal_end','cal_end','','cmd');
		$status_id = $mainframe->getUserStateFromRequest('affiliate.conversions.status_id','status_id','','cmd');
		$user_id = $mainframe->getUserStateFromRequest('affiliate.conversions.user_id','user_id','','int');
		$target_user_id = $mainframe->getUserStateFromRequest('affiliate.conversions.target_user_id','target_user_id','','int');
		$account_id = $mainframe->getUserStateFromRequest('affiliate.conversions.account_id','account_id','','int');
		...
		$this->setState('target_user_id', $target_user_id);
	...
	function getTargetUserId(){
		if (empty($this->target_user_id)) {
			$this->target_user_id = $this->getState('target_user_id')	;
		}
	...
	function _buildQuery(){
		...
		$search = '%' . $this->_db->escape( $keywords, true ) . '%' ;

		$where_clause = array();

		if ($keywords != ""){
			$where_clause[] = ' ( at.name LIKE '.$this->_db->quote( $search, false ).' OR at.extended_name LIKE '.$this->_db->quote( $search, false )
			.' OR u.username LIKE '.$this->_db->quote( $search, false ).' OR u.name LIKE '.$this->_db->quote( $search, false )
			.' OR u.email LIKE '.$this->_db->quote( $search, false ).' OR acc.account_name LIKE '.$this->_db->quote( $search, false ).') ';
		}
		if ($cal_start != ""){
			$where_clause[] = ' at.date_created >= '.$this->_db->quote( $cal_start );
		}
		if ($cal_end != ""){
			$where_clause[] = ' at.date_created <= '.$this->_db->quote( $cal_end );
		}
		if ($status_id !== ""){
			$where_clause[] = ' at.approved = "'.(int)$status_id.'" ';
		}
		if ($user_id != ""){
			$where_clause[] = ' acc.user_id = "'.$user_id.'" ';
		}
		if ($target_user_id != ""){
			$where_clause[] = ' at.user_id = "'.$target_user_id.'" ';
		}
		if ($account_id != ""){
