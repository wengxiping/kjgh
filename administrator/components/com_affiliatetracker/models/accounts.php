...
class AccountsModelAccounts extends JModelLegacy
{
	...
	function __construct(){
		...
		$cal_start = $mainframe->getUserStateFromRequest('affiliate.accounts.cal_start','cal_start','','cmd');
		$cal_end = $mainframe->getUserStateFromRequest('affiliate.accounts.cal_end','cal_end','','cmd');
		$status_id = $mainframe->getUserStateFromRequest('affiliate.accounts.status_id','status_id','','cmd');
		$user_id = $mainframe->getUserStateFromRequest('affiliate.accounts.user_id','user_id','','int');
	...
	function _buildQuery(){
		...
		if ($keywords != ""){
			$where_clause[] = ' ( acc.account_name LIKE '.$this->_db->quote( $search, false ).' OR u.username LIKE '.$this->_db->quote( $search, false ).' OR u.name LIKE '.$this->_db->quote( $search, false ).' OR u.email LIKE '.$this->_db->quote( $search, false ).' ) ';
		}
		if ($status_id !== ""){
			$where_clause[] = ' acc.publish = "'.(int)$status_id.'" ';
		}
		if ($user_id != ""){
